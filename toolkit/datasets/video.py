...
class Video(object):
    def __init__(self, name, root, video_dir, init_rect, img_names,
        ...
        tmp = [os.path.join(os.path.dirname(x), "color", os.path.basename(x)) for x in img_names]
        self.img_names = [os.path.join(os.path.abspath(root), x) for x in tmp]
        # self.img_names = [os.path.join(os.path.abspath(root), x.split('/')[0], x.split('/')[-1]) for x in img_names]
        self.imgs = None
