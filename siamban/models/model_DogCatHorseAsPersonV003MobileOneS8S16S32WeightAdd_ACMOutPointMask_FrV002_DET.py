...
from siamban.models.fcos_loss import make_fcos_loss_evaluator
...
class FCOSHead(torch.nn.Module):
    def __init__(self, cfg, in_channels):
        ...
        num_classes = cfg.TRAIN.FCOS.NUM_CLASSES - 1

        cls_tower = []
        bbox_tower = []
        for i in range(cfg.TRAIN.FCOS.NUM_CONVS):
            conv_func = nn.Conv2d
            cls_tower.append(
                conv_func(
                    in_channels,
        ...
        prior_prob = cfg.TRAIN.FCOS.PRIOR_PROB
...
class ModelBuilder(nn.Module):
    def __init__(self,cfg=None,world_size=None):
        ...
        #build FCOS head
        self.fcos_head = FCOSHead(cfg=cfg, in_channels=192)
        self.fcos_loss = make_fcos_loss_evaluator(cfg=cfg, world_size=world_size)
    ...
    def detection(self, d):
        ...
        df = self.backbone(d)
        df = self.neck(df)
        det_cls, det_loc = self.fcos_head(df)
        det_loc *= 8.0
        return {
            'detection_cls': det_cls,
            'detection_loc': det_loc
        }
    ...
    def forward(self, data,rank=None):
        ...
        if rank is None:
            ...
            detection = data['detection'].cuda().float()
            ...
            label_target = data['searchboxes'].float().to(rank, non_blocking=True).flatten(0,1)
            detection = data['detection'].float().to(rank, non_blocking=True)
            detection_bboxes = data['detection_bboxes']
        ...
        # get feature
        # template = template[:, 0, :, :, :]
        # template = template.squeeze(dim=1)
        zf = self.backbone(template)
        xf = self.backbone(search)
        df = self.backbone(detection)
        ...
        if cfg.TRAIN.RANK_IGR_WEIGHT > 0:
            ...
            # except:
            #     pass
        if cfg.TRAIN.FCOS_CLS_WEIGHT>0:
            det_locations = self.compute_locations([df, ])
            FCOS_cls_loss, FCOS_loc_loss = self.fcos_loss(det_locations, [det_cls,], [det_loc,], detection_bboxes)
            outputs['total_loss'] += (cfg.TRAIN.FCOS_CLS_WEIGHT*FCOS_cls_loss + cfg.TRAIN.FCOS_LOC_WEIGHT * FCOS_loc_loss)
            outputs['FCOS_cls_loss'] = FCOS_cls_loss
            outputs['FCOS_loc_loss'] = FCOS_loc_loss
        return outputs
...
if __name__=='__main__':
    ...
    from siamban.core.config import cfg
    # backbone = mobileone(variant='s0')
    # backbone = mobileones16outtwo(variant='s0')
    # cfg.merge_from_file(
    #     "/home/lbycdy/work/siamban-acm/experiments/siamban_r50_l234/20221121DogCatHorseAsPersonV003MobileOneS8S16S32WeightAdd_ACMOutPointMaskVidImgRandSIn224_4GPU125.yaml")
    cfg.merge_from_file('/home/lbycdy/work/siamban/experiments/siamban_r50_l234/20221201DogCatHorseAsPersonV003MobileOneS8S16S32WeightAdd_ACMOutPointMaskVidImgRandSIn224.yaml')
    model = ModelBuilder(cfg).cuda()
    checkpoint = torch.load('/home/lbycdy/checkpoint/mobileone_s0_unfused.pth.tar')
    # cfg.merge_from_file(
    #     "/home/lbycdy/work/siamban-acm/experiments/siamban_r50_l234/20221121DogCatHorseAsPersonV003MobileOneS8S16S32WeightAdd_ACMOutPointMaskVidImgRandSIn224_4GPU125.yaml")
    # model = mobileone(variant='s0')
    model.backbone.load_state_dict(checkpoint)
    model.backbone = reparameterize_model_allskipscale(model.backbone).cuda()
    data = {}
    # from siamban.core.config import cfg
    ...
    #
    # data = {}
    data['template'] = torch.randn((4, 3, 160, 160)).cuda()
    data['search'] = torch.randn((4, 5, 3, 160, 160)).cuda()
    data['label_loc'] = torch.randn((4, 5, 4)).cuda()
    data['label_cls'] = torch.randn((4, 5, 1)).cuda()
    data['searchboxes'] = torch.randn((4, 5, 4)).cuda()
    data['detection'] = torch.randn((4, 3, 224,224)).cuda()
    data['detection_bboxes'] = torch.randn((4, 5, 4)).cuda()
    out = model(data)