...
if uname=='ethan':
    ...
    from utils.structures.boxlist_ops import cat_boxlist
    from utils.structures.boxlist_ops import boxlist_ml_nms
    from utils.structures.boxlist_ops import remove_small_boxes
...
class AdjustAllLayerFPNAddFlagCenterCrop(nn.Module):
    def __init__(self, in_channels, out_channels, center_size=9):
        ...
        self.ca3 = ChannelAttention(out_channels[1], 128)
    ...
    def forward(self, features,flagcentercrop=False):
        ...
        downsample3 = self.ca2(downsample3) * downsample3
...
class AdjustAllLayerFPNAddFlagCenterCropTwoIn(nn.Module):
    def __init__(self, in_channels, out_channels, center_size=9):
        super(AdjustAllLayerFPNAddFlagCenterCropTwoIn, self).__init__()
        self.center_size = center_size
        self.downsample1 = nn.Sequential(
            nn.Conv2d(in_channels[0], out_channels[0], kernel_size=1, bias=False),
            nn.BatchNorm2d(out_channels[0]),
...
class RPNHead(nn.Module):
    def __init__(self,cin = 256):
        ...
        self.head1 = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            nn.Conv2d(feature_in, feature_in, kernel_size=3, bias=False, padding=1,dilation=1),
            nn.BatchNorm2d(feature_in),
            nn.ReLU(inplace=True),
        ...
        self.cls = nn.Sequential(
            nn.Conv2d(feature_in, feature_in,kernel_size=3, bias=False,padding=0,stride=2),
            nn.BatchNorm2d(feature_in),
            nn.ReLU(inplace=True),
            nn.Conv2d(feature_in, feature_in, kernel_size=3, bias=False, padding=1,dilation=1),
            nn.BatchNorm2d(feature_in),
...
class ModelBuilder(nn.Module):
    def __init__(self,cfg=None):
        ...
        # build adjust layer
        self.neck = AdjustAllLayerFPNAddFlagCenterCropTwoIn(in_channels=[256,1024],out_channels=[outch,outch],center_size=7)
        self.neckconcat = AdjustAllLayerFPNAddFlagCenterCrop(in_channels=[128,256,1024],out_channels=[outch,outch,outch],center_size=7)
    ...
    def track(self, x):
        ...
        cls, loc = self.head(xf, self.zf,xfconcat)
        cls_det,loc_det = self.dethead(xfconcat)
        print(cls_det.size(),cls.size(),":cls_det:cls ")
    ...
    def forward(self, data,rank=None,numgpus=1):
        ...
        zf = self.neck(zf[1:], flagcentercrop=True)
        xfcorr = self.neck(xf[1:])
        xfconcat = self.neckconcat(xf)
        ...
        N,C,H,W = xfcorr.size()
        xfcorr = xfcorr.view((Nbatch,Ns,C,H,W))
        xfcorr = torch.unsqueeze(xfcorr, dim=1)
        xfcorr = torch.tile(xfcorr, [1, Nt, 1, 1, 1,1])
        xfcorr = xfcorr.flatten(0, 2)

        xfconcat = xfconcat.view((Nbatch, Ns, C, H, W))
        xfconcat = torch.unsqueeze(xfconcat, dim=1)
        xfconcat = torch.tile(xfconcat, [1, Nt, 1, 1, 1, 1])
        xfconcat = xfconcat.flatten(0, 2)

        cls_corr,loc_corr = self.head(xfcorr,zf,xfconcat)
        # cls_corr = self.log_softmax(cls_corr)
        cls_corr = cls_corr.permute(0, 2, 3, 1).contiguous()
        cls_loss_corr = select_cross_entropy_loss(cls_corr, label_cls_corr, True, rank=rank,flaguselogits=True)
