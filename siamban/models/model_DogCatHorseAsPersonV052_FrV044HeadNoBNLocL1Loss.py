...
from siamban.models.rank_loss import select_cross_entropy_loss, select_iou_loss, rank_cls_loss,rank_loc_loss,rank_loc_loss_fast,rank_center_loc_loss,select_l1_loss
...
class RPNHead(nn.Module):
    def __init__(self,cin = 256,p=0.0,pspatial=0.0,flagexp=True):
        super(RPNHead, self).__init__()
        self.depthcorr = DepthwiseXCorr(cin,cin)
        self.flagexp = flagexp
        feature_in = 32
        self.head1 = nn.Sequential(
            nn.Conv2d(cin*2, feature_in*2, kernel_size=3, bias=True, padding=1),
            nn.ReLU(inplace=True),
            nn.Conv2d(feature_in*2, feature_in, kernel_size=3, bias=True, padding=2,dilation=2),
            nn.ReLU(inplace=True),
            nn.Conv2d(feature_in, feature_in, kernel_size=3, bias=True, padding=2,dilation=2),
            nn.ReLU(inplace=True),
            nn.Conv2d(feature_in, feature_in, kernel_size=3, bias=True, padding=2,dilation=2),
            nn.ReLU(inplace=True),
            nn.Conv2d(feature_in, feature_in, kernel_size=3, bias=True, padding=2,dilation=2),
...
class ModelBuilder(nn.Module):
    def __init__(self,cfg=None):
        ...
        # build rpn head
        self.head = RPNHead(cin=192,p=0.0,pspatial=0.0,flagexp=cfg.TRAIN.FLAG_LOC_EXP)
