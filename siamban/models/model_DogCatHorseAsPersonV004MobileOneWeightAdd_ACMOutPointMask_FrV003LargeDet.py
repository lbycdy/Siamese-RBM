...
from siamban.models.loss_multiobj import FCOSAlikeLossComputation_TargetIn
...
class ModelBuilder(nn.Module):
    def __init__(self,cfg=None):
        ...
        self.depthcorr = DepthwiseXCorr(192,192)


        # build rpn head
        self.head = RPNHead(cin=192)
        self.dethead = DetHead(cin=192)
        ...
        self.loss_det = FCOSAlikeLossComputation_TargetIn(cfg)
    ...
    def forward(self, data,rank=None,num_gpus=1):
        """ only used in training
        """
        Nbatch, Nperbatch = data['search'].size()[:2]
        Nbatch, Nperbatch_det = data['search_det'].size()[:2]
        # print(data['search_det'].size(),"beforefla")
        if rank is None:
            ...
            search_det = data['search_det'].cuda().float().flatten(0,1)
            cls_det = data['cls_det'].cuda().float().flatten(0,1)
            reg_det = data['reg_det'].cuda().float().flatten(0,1)
            ...
            search_det = data['search_det'].float().to(rank, non_blocking=True).flatten(0,1)
            cls_det = data['cls_det'].float().to(rank, non_blocking=True).flatten(0,1)
            reg_det = data['reg_det'].float().to(rank, non_blocking=True).flatten(0,1)

        # print(search_det.size(),template.size(),search.size(),label_loc.size(),label_cls.size())
        #orch.Size([16, 3, 256, 256]) torch.Size([4, 3, 160, 160]) torch.Size([60, 3, 224, 224])
        ...
        if cfg.DATASET.CATDOGHORSETK.NBATCH_SHAPREPREV>1:
            ...
            for i in range(nstep):
                idx = i * cfg.DATASET.CATDOGHORSETK.NBATCH_SHAPREPREV + random.randint(0, cfg.DATASET.CATDOGHORSETK.NBATCH_SHAPREPREV - 1)
        ...
        xf_det = self.backbone(search_det)
        zf = self.neck(zf, flagcentercrop=True)
        zf_tk = torch.unsqueeze(zf, dim=1)
        zf_tk = torch.tile(zf_tk, [1, Nperbatch*cfg.DATASET.CATDOGHORSETK.NBATCH_SHAPREPREV, 1, 1, 1])
        zf_tk = zf_tk.flatten(0, 1)
        xf = self.neck(xf)
        depthcorr = self.depthcorr(xf,zf_tk)
        cls,loc = self.head(depthcorr)

        xf_det = self.neck(xf_det)
        zf_det = torch.unsqueeze(zf, dim=1)
        zf_det = torch.tile(zf_det, [1, Nperbatch_det*cfg.DATASET.CATDOGHORSETK.NBATCH_SHAPREPREV, 1, 1, 1])
        zf_det = zf_det.flatten(0, 1)
        depthcorr_det = self.depthcorr(xf_det, zf_det)

        cls_det_pred,loc_det_pred = self.dethead(depthcorr_det)
        ...
        # print(cls_det_pred.size(),loc_det_pred.size(),cls_det.size(),reg_det.size())
        # exit()

        cls_loss_det, reg_loss_det = self.loss_det(cls_det, reg_det, cls_det_pred, loc_det_pred, num_gpus)

        outputs = {}
        outputs['total_loss'] = cfg.TRAIN.CLS_WEIGHT * cls_loss + \
                                cfg.TRAIN.LOC_WEIGHT * loc_loss + cls_loss_det + reg_loss_det
        outputs['cls_loss'] = cls_loss.detach()
        outputs['loc_loss'] = loc_loss.detach()
        outputs['cls_loss_det'] = cls_loss_det.detach()
        outputs['reg_loss_det'] = reg_loss_det.detach()
...
if __name__=='__main__':
    from siamban.core.config import cfg
    cfg.merge_from_file("/home/ethan/work/siamban-acm/experiments/siamban_r50_l234/20221116DogCatHorseAsPersonV002MobileOneS8S16S32WeightAdd_ACMOutPointMaskSIn224_4GPU127.yaml")
    model = ModelBuilder(cfg).cuda()
    data = {}
    data['template'] = torch.randn((1,5,3,160,160)).cuda()
