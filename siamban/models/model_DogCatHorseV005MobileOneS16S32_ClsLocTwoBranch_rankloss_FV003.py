...
class AdjustAllLayerFlagCenterCrop(nn.Module):
    ...
    def forward(self, features,flagcentercrop=False):
        ...
        if flagcentercrop:
            ...
            downsample2 = downsample2[:, :, l:r, l:r]

        return [downsample1,downsample2]


class DepthwiseXCorr(nn.Module):
    def __init__(self, in_channels, hidden, kernel_size=3,cfg=None):
        ...
        elif cfg.TRAIN.CONVTYPE_XORR == 1:
            self.xorr_activate = nn.Sequential(
                nn.Conv2d(hidden, hidden, kernel_size=3, bias=True, padding=1),
                nn.BatchNorm2d(hidden),
                nn.ReLU(inplace=True)
    ...
    def forward(self, kernel,search):
        ...
        if self.cfg.TRAIN.TYPE_ACM==0:
            f = search_part + kernel_part
        elif self.cfg.TRAIN.TYPE_ACM==1:
            f = search_part*kernel_part
        feature = self.xorr_activate(f)
        return feature


class MutltiBAN(nn.Module):
    def __init__(self, in_channels=256, out_channels=256,cfg=None):
        super(MutltiBAN, self).__init__()
        self.cfg = cfg
        for i in range(2):
            self.add_module('box' + str(i + 2), DepthwiseXCorr(in_channels, out_channels, kernel_size=3,cfg=cfg))

        self.headcls = nn.Sequential(
            nn.Conv2d(out_channels, out_channels, kernel_size=3, bias=True, padding=1),
        ...
        self.headloc = nn.Sequential(
            ...
            nn.BatchNorm2d(out_channels),
            nn.ReLU(inplace=True),
            nn.Conv2d(out_channels, 4, kernel_size=3, padding=1)
        )
        self.feat_weight = nn.Parameter(torch.ones(2))

    def forward(self, z_fs,x_fs):
        feat_weight = F.softmax(self.feat_weight, 0)
        feat = 0
        for idx, (z_f, x_f) in enumerate(zip(z_fs, x_fs), start=2):
            box = getattr(self, 'box' + str(idx))
            f = box(z_f, x_f)
            feat += feat_weight[idx-2]*f
        cls = self.headcls(feat)
        loc = self.headloc(feat)
        loc = torch.exp(loc)
        return cls,loc
...
class ModelBuilder(nn.Module):
    def __init__(self,cfg):
        ...
        # build ban head
        self.head = MutltiBAN(in_channels=channel, out_channels=channel,cfg=cfg)
    ...
    def forward(self, data,rank=None):
        ...
        if self.cfg.TRAIN.RANK_IGR_WEIGHT>0:
            ...
            outputs['total_loss'] = outputs['total_loss']  +self.cfg.TRAIN.RANK_IGR_WEIGHT*IGR_loss_1+self.cfg.TRAIN.RANK_IGR_WEIGHT*IGR_loss_2
