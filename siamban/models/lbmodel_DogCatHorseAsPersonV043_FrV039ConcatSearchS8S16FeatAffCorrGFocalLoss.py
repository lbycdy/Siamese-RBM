...
from siamban.models.rank_loss import select_cross_entropy_loss, select_iou_loss, rank_cls_loss,rank_loc_loss,rank_loc_loss_fast,rank_center_loc_loss, quality_focal_loss
...
class RPNHead(nn.Module):
    def __init__(self,cin = 256,p=0.0,pspatial=0.0):
    ...
    def forward(self, xfs16s32,zf,xfs8s16):
        depthcorr = self.depthcorr(xfs16s32,zf)
        depthcorr = self.drop(depthcorr)

        head1 = self.head1(torch.cat([depthcorr,xfs8s16],dim=1))
        n, c, h, w = head1.size()
...
class ModelBuilder(nn.Module):
    def __init__(self,cfg=None):
        ...
        if cfg.BACKBONE.VARIANT=='s0':
            ...
            # build adjust layer
            self.neck1 = AdjustAllLayerFPNAddFlagCenterCrop(in_channels=[256,1024],out_channels=[192,192],center_size=9,weight=cfg.TRAIN.BBADDWEIGHT)
            self.neck2 = AdjustAllLayerFPNAddFlagCenterCrop(in_channels=[128, 256], out_channels=[192, 192],center_size=9, weight=cfg.TRAIN.BBADDWEIGHT)
        else:
            self.backbone = mobileones16outtwo(variant='s1')
            # build adjust layer
            self.neck = AdjustAllLayerFPNAddFlagCenterCrop(in_channels=[512, 1280], out_channels=[192, 192],center_size=9, weight=cfg.TRAIN.BBADDWEIGHT)
            self.neck = AdjustAllLayerFPNAddFlagCenterCrop(in_channels=[192, 512], out_channels=[192, 192],center_size=9, weight=cfg.TRAIN.BBADDWEIGHT)

        # build rpn head
        self.head = RPNHead(cin=192,p=0.0,pspatial=0.0)
    ...
    def template(self, z):
        ...
        zf = self.neck1(zf[1:], flagcentercrop=True)
        self.zf = zf
    ...
    def track(self, x):
        ...
        featbb =  self.neck2(xf[:2])
        xf_corr = self.neck1(xf[1:])

        cls, loc = self.head(xf_corr, self.zf,featbb)
    ...
    def forward(self, data,rank=None):
        ...
        zf = zf.flatten(0, 1)
        xfs16s32 = self.neck1(xfbb[1:])
        xfs8S16 = self.neck2(xfbb[:2])
        cls, loc = self.head(xfs16s32,zf,xfs8S16)
        ...
        cls_loss = quality_focal_loss(cls, label_cls,label_loc, rank,cfg.TRAIN.FOCAL_ALPHA,cfg.TRAIN.FOCAL_GAMA)
