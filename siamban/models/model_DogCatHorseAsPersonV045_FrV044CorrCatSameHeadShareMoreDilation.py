...
class ModelBuilder(nn.Module):
    def __init__(self,cfg=None):
        ...
        self.cfg = cfg
        chann = cfg.BAN.channels
        # build backbone
        if cfg.BACKBONE.VARIANT=='s0':
            self.backbone = mobileones16outtwo(variant='s0')
            # build adjust layer
            self.neck = AdjustAllLayerFPNAddFlagCenterCrop(in_channels=[128,256,1024],out_channels=[chann,chann,chann],center_size=9,weight=cfg.TRAIN.BBADDWEIGHT)
        else:
            self.backbone = mobileones16outtwo(variant='s1')
            # build adjust layer
            self.neck = AdjustAllLayerFPNAddFlagCenterCrop(in_channels=[192, 512, 1280], out_channels=[chann, chann, chann],
                                                           center_size=9, weight=cfg.TRAIN.BBADDWEIGHT)


        # build rpn head
        self.head = RPNHead(cin=chann,p=0.0,pspatial=0.0,flagexp=cfg.TRAIN.FLAG_LOC_EXP)
