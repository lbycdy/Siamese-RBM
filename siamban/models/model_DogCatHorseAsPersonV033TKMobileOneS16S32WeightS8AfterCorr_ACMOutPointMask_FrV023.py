...
class RPNHead(nn.Module):
    def __init__(self,cin = 256,cinf8=128,p=0.0):
...
class MobileOneS0Det(nn.Module):
    def __init__(self):
        super(MobileOneS0Det, self).__init__()
        net_tmp = mobileones16outtwo(variant="s0")
        self.init_block = net_tmp.stage0
        self.stage1 = net_tmp.stage1
        self.stage2 = net_tmp.stage2
        self.stage3 = net_tmp.stage3
        self.stage4 = net_tmp.stage4[0]

    def forward(self, x):
        x2 = self.init_block(x)
        x4 = self.stage1(x2)
        x8 = self.stage2(x4)
        x16 = self.stage3(x8)
        x32 = self.stage4(x16)
        # print(x2.size(),x4.size(),x8.size(),x16.size(),x32.size())
        return [x8,x16,x32]
...
class ModelBuilder(nn.Module):
    def __init__(self,cfg=None):
        ...
        self.cfg = cfg
        # build backbone
        self.backbone = MobileOneS0Det()
        # build adjust layer

        self.neck = AdjustAllLayerFPNAddFlagCenterCrop(in_channels=[256,256],out_channels=[192,192],center_size=9)

        # build rpn head
        self.head = RPNHead(cin=192,p=cfg.TRAIN.PROB_DROPOUT)
        ...
        self.mean = torch.tensor([103.53, 116.28, 123.675]).view((1, 3, 1, 1))
        self.std = torch.tensor([57.375, 57.12, 58.395]).view((1, 3, 1, 1))

    def template(self, z):
        netin = z.clone()
        if self.cfg.TRAIN.DATA_NORMALIZE_MODE == 1:
            netin -= 114.0
            netin /= 58.0
        elif self.cfg.TRAIN.DATA_NORMALIZE_MODE==2:
    ...
    def track(self, x):
        ...
        print(self.cfg.TRAIN.NORM_REG_TARGETS)
    ...
    def forward(self, data,rank=None):
        ...
        if self.cfg.TRAIN.DATA_NORMALIZE_MODE==1:
            ...
            self.mean = self.mean.to(rank)
            self.std = self.std.to(rank)
            template -= self.mean
            template /= self.std
            search -= self.mean
            search /= self.std
        ...
        zf = zf.flatten(0, 1)
        xf = self.neck(xfbb[1:])
        cls, loc = self.head(xf,zf,xfbb[0])
