...
class RPNHead(nn.Module):
    def __init__(self,cin = 256,cinf8=128,cinf4=48,p=0.0,pspatial=0.0,flagclassoncorr=False):
        ...
        self.weight = nn.Parameter(torch.ones(2))
        self.downsamplef8 = nn.Sequential(
            nn.Conv2d(cinf8, cinf8, kernel_size=1, bias=False),
            nn.BatchNorm2d(cinf8),
        )
        self.downsamplef4 = nn.Sequential(
            nn.Conv2d(cinf4, cinf8, 3, bias=False,stride=2),
            nn.BatchNorm2d(cinf8),
        )
    ...
    def forward(self, xf,zf,f4,f8):
        ...
        weight = F.softmax(self.weight, 0)
        downsamplef8 = self.downsamplef8(f8)
        downsamplef4 = self.downsamplef4(f4)
        # print(f4.size(),downsamplef4.size())
        featlow = downsamplef8*weight[0]+downsamplef4*weight[1]

        featcat = torch.cat([depthcorr,featlow],dim=1)
...
class ModelBuilder(nn.Module):
    def __init__(self,cfg=None):
        ...
        self.cfg = cfg
        # build backbone
        self.backbone = mobileones16outtwo(variant='s0',flagouts4 = True)
        # build adjust layer
