...
class RPNHead1(nn.Module):
    def __init__(self,cin = 256):
        super(RPNHead1, self).__init__()
        self.depthcorr = DepthwiseXCorr(cin,cin)
        feature_in = 32
...
class ModelBuilder(nn.Module):
    ...
    def forward(self, data,rank=None):
        ...
        if rank is None:
            ...
            label_loc1 = data['label_loc1'].cuda().float().flatten(0,1)
        else:
        ...
        loc_loss1 = (loc1 - label_loc1).abs().sum()
        outputs = {}
        outputs['total_loss'] = cfg.TRAIN.CLS_WEIGHT * cls_loss + \
                                cfg.TRAIN.LOC_WEIGHT * loc_loss + 0.001*loc_loss1
        outputs['cls_loss'] = cls_loss.detach()
        outputs['loc_loss'] = loc_loss.detach()
        outputs['loc_loss1'] = loc_loss1.detach()
        """
