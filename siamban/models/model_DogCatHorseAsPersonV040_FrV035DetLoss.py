...
from siamban.models.loss_multiobj import FCOSAlikeLossComputationOneClassAndBack
...
class AdjustAllLayerFPNAddFlagCenterCrop(nn.Module):
    ...
    def forward(self, features,flagcentercrop=False):
        ...
        out = downsample1*self.weight[0]+downsample2*self.weight[1]+downsample3*self.weight[2]
        if flagcentercrop:
...
class ModelBuilder(nn.Module):
    def __init__(self,cfg=None):
        ...
        self.loss_det = FCOSAlikeLossComputationOneClassAndBack(cfg)
    ...
    def forward(self, data,rank=None,numgpus=4):
        ...
        if rank is None:
            ...
            searchboxes = [box.cuda().float() for box in data['searchboxes']]

        else:
            template = data['template'].float().to(rank, non_blocking=True)
            search = data['search'].float().to(rank, non_blocking=True).flatten(0,1)
            searchboxes = [box.cuda().float().to(rank, non_blocking=True) for box in data['searchboxes']]
        ...
        location = self.compute_locations_per_level(cls,8,imgsize//2)
        cls_loss, loc_loss,_,labels_corr = self.loss_det(location, [cls,], [loc,], searchboxes,numgpus,centerness=None)
