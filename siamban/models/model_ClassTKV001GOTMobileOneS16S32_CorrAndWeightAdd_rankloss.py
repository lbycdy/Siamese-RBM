...
class MutltiBAN(nn.Module):
    ...
    def forward(self, z_fs,x_fs):
        ...
        for idx, (z_f, x_f) in enumerate(zip(z_fs, x_fs), start=2):
            ...
            c, l = box(z_f, x_f)
            cls += cls_weight[idx-2]*c
            if self.cfg.TRAIN.FLAG_LOCPREDMODE==0:
                loc += loc_weight[idx-2]*l * self.loc_scale[idx - 2]
            elif self.cfg.TRAIN.FLAG_LOCPREDMODE==1:
                loc += loc_weight[idx - 2] * torch.exp(l * self.loc_scale[idx - 2])
        if self.cfg.TRAIN.FLAG_LOCPREDMODE == 0:
            loc = torch.exp(loc)
        return cls,loc
