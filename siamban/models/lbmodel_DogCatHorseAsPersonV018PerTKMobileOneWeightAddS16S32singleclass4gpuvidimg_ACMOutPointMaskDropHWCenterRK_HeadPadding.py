...
from siamban.models.rank_loss1 import select_cross_entropy_loss, select_iou_loss, rank_cls_loss,rank_loc_loss
from siamban.models.backbone.mobileone_stride import mobileone
from siamban.models.backbone.mobileone_strideS16S32OutTwo import mobileone as mobileones16outtwo
...
class RPNHead(nn.Module):
    def __init__(self,cin = 256):
        ...
        self.drop1 = nn.Dropout2d(p=cfg.BACKBONE.DROPOUTHW)
    def forward(self, xf,zf):
        depthcorr = self.depthcorr(xf,zf)
        depthcorr = self.drop(depthcorr)
        head1 = self.head1(depthcorr)
        n, c, h, w = head1.size()
        head1hw = head1.view(n,c,h*w).transpose(1,2).reshape(n,h*w,c,1)
        head1hw = self.drop1(head1hw).transpose(1,2).view(n,c,h,w)
        cls = self.cls(head1hw)
        loc = self.loc(head1hw)
...
class ModelBuilder(nn.Module):
    def __init__(self,cfg=None):
        ...
        # self.rank_center_loc_loss = rank_center_loc_loss(cfg)
    def template(self, z):
        temp_z = z.clone()
        temp_z -= 114.0
        temp_z /= 58.0
    ...
    def forward(self, data,rank=None):
        ...
        if rank is None:
            ...
            img_vid_flag = data['img_vid_flag'].to(rank,non_blocking=True)
        ...
        loc_loss = select_iou_loss(loc, label_loc, label_cls,img_vid_flag, rank=rank)
        outputs = {}
        ...
        if cfg.TRAIN.RANK_IGR_WEIGHT>0:
            ...
            IGR_loss_1, IGR_loss_2 = self.rank_center_loc_loss(cls, label_cls,label_loc, pred_bboxes, label_target,
                                                        flagsigmoid=cfg.TRAIN.FLAG_SIGMOID_LOSS, rank=rank)
            # try:
