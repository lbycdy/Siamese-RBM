...
class AdjustAllLayerFlagCenterCrop(nn.Module):
    ...
    def forward(self, features,flagcentercrop=False):
        ...
        if flagcentercrop:
            ...
            downsample3 = downsample3[:, :, l:r, l:r]

        return [downsample1,downsample2,downsample3]
...
class MultiBAN(nn.Module):
    def __init__(self, in_channels, cls_out_channels, weighted=False):
        super(MultiBAN, self).__init__()
        self.weighted = weighted
        for i in range(len(in_channels)):
            self.add_module('box'+str(i), DepthwiseBAN(in_channels[i], in_channels[i], cls_out_channels))
        if self.weighted:
    ...
    def forward(self, z_fs,x_fs):
        ...
        for idx, (z_f, x_f) in enumerate(zip(z_fs, x_fs)):
            ...
            c, l = box(z_f,x_f)
            cls.append(c)
            loc.append(torch.exp(l*self.loc_scale[idx]))
...
class ModelBuilder(nn.Module):
    def __init__(self,cfg):
        ...
        # build adjust layer
        self.neck = AdjustAllLayerFlagCenterCrop(in_channels=[128,256,1024],out_channels=[channel,channel,channel],center_size=7)

        # build ban head
        self.head = MultiBAN(in_channels=[channel]*3, cls_out_channels=2)
