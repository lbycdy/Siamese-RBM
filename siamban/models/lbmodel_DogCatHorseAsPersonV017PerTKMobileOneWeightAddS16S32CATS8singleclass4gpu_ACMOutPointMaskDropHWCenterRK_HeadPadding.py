...
from siamban.models.rank_loss import select_cross_entropy_loss, select_iou_loss, rank_cls_loss,rank_loc_loss,rank_center_loc_loss
...
class RPNHead(nn.Module):
    def __init__(self,cin = 256,cinf8 = 128):
    ...
    def forward(self, xf,zf,searchf8):
        depthcorr = self.depthcorr(xf,zf)
        #initialization
        depthcorr = self.drop(depthcorr)
        f = torch.cat([searchf8, depthcorr], dim=1)
        head1 = self.head1(f)
...
class ModelBuilder(nn.Module):
    ...
    def track(self, x):
        ...
        xf1 = self.neck(xf[1:])
        cls, loc = self.head(xf1, self.zf,xf[0])
