...
class MobileOneBlock(nn.Module):
    ...
    def __init__(self,
        ...
        self.num_conv_branches = num_conv_branches  #3*3卷积个数
        if stride==2:
            padding = 0
    ...
    def reparameterize_train(self):
        ...
        kernel, bias = self._get_kernel_bias_train()
        module_new = self._conv_bn(kernel_size=self.rbr_conv[0].conv.kernel_size,
                                   padding=self.rbr_conv[0].conv.padding)
        module_new.conv.weight.data = kernel
        module_new.bn.bias.data = bias
        self.rbr_conv[0] = module_new
        self.rbr_skip = None
        self.rbr_scale = None

        # if self.stride==2:
        #     kernel, bias = self._get_kernel_bias_train()
        #     module_new = self._conv_bn(kernel_size=3, padding=0)
        #     module_new.conv.weight.data = kernel
        #     module_new.bn.bias.data = bias
        #     self.rbr_conv[0] = module_new
        #     self.rbr_scale = None
        # elif self.stride==1 and self.kernel_size==3:
        #     kernel, bias = self._get_kernel_bias_train()
        #     module_new = self._conv_bn(kernel_size=3, padding=1)
        #     module_new.conv.weight.data = kernel
        #     module_new.bn.bias.data = bias
        #     self.rbr_conv[0] = module_new
        #     self.rbr_scale = None
        ...
        # else:
        #     kernel, bias = self._get_kernel_bias_train()
        #     module_new = self._conv_bn(kernel_size=1, padding=0)
        #     module_new.conv.weight.data = kernel
        #     module_new.bn.bias.data = bias
        #     self.rbr_conv[0] = module_new
        #     self.rbr_skip = None
    ...
    def _get_kernel_bias_train(self) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    def _conv_bn(self,
                 kernel_size: int,
                 padding: int) -> nn.Sequential:          #定义一个普通的conv2d和一个BN层
...
class MobileOne(nn.Module):
    ...
    def __init__(self,
        ...
        self.stage3 = self._make_stage(int(256 * width_multipliers[2]), num_blocks_per_stage[2],
                                       num_se_blocks=int(num_blocks_per_stage[2] // 2) if use_se else 0,flagspecial=True)
        self.stage4 = self._make_stage(int(512 * width_multipliers[3]), num_blocks_per_stage[3],
                                       num_se_blocks=num_blocks_per_stage[3] if use_se else 0,flagspecial=True)
    ...
    def _make_stage(self,
                    ...
                    num_blocks: int,
                    num_se_blocks: int,
                    flagspecial: bool=False) -> nn.Sequential:
        ...
        for ix, stride in enumerate(strides):
            ...
            blocks.append(MobileOneBlock(in_channels=self.in_planes,
                                         ...
                                         flagspecial=flagspecial and ix==0))
            # Pointwise conv
            blocks.append(MobileOneBlock(in_channels=self.in_planes,
                                         out_channels=planes,
                                         kernel_size=1,
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """ Apply forward pass. """
        x = self.stage0(x) #[1,48,79,79]
        print(x.size())
        x = self.stage1(x)#[1,48,39,39]
        print(x.size())
        x = self.stage2(x)#[1,128,19,19]
        print(x.size())
        x8 = x
        for i in range(11):
            x = self.stage3[i](x)#[1,128,19,19]*1
            print(x.size())      #[1,256,19,19]*10
        x16 = x
        for i in range(11,20):
            x = self.stage3[i](x)#[1,256,19,19]*10
            print(x.size())
        x = self.stage4(x)#[1,1024,19,19]
        print(x.size())
        x32 = x
        return [x8,x16,x32]
...
def reparameterize_model_train(model: torch.nn.Module) -> nn.Module:
    ...
    for module in model.modules():
        if hasattr(module, 'reparameterize_train'):
            module.reparameterize_train()
    return model

def reparameterize_models2(model: torch.nn.Module) -> nn.Module:
    """ Method returns a model where a multi-branched structure
        used in training is re-parameterized into a single branch
        for inference.
...
def reparameterize_models_all(model: torch.nn.Module) -> nn.Module:
    """ Method returns a model where a multi-branched structure
        used in training is re-parameterized into a single branch
        for inference.
...
if __name__=="__main__":
    ...
    model = reparameterize_model_train(model).eval()

    model1 = reparameterize_models_all(model1)

    img = torch.randn((1, 3, 160, 160))
    out = model(img)
    out1 = model1(img)
    for i in range(3):
        diff = torch.abs(out1[i]-out[i])
        print(diff.min(),diff.max())
    # for o in out:
    #     print(o.size())
    # for key in model_eval.state_dict():
    #     print(key)
    # torch_out = torch.onnx._export(model_eval,img,"mobileonestride.onnx",export_params=True,input_names=["template"],output_names=["zf"])
