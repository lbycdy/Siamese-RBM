...
class MobileOneBlock(nn.Module):
    ...
    def reparameterize_train(self):
        ...
        module_new = self._conv_bn(kernel_size=3, padding=self.rbr_conv[0].conv.padding)
...
class MobileOne(nn.Module):
    ...
    def __init__(self,
                 ...
                 num_classes: int = 1000,flagouts4=False,
...
def mobileone(num_classes: int = 1000, inference_mode: bool = False,
              variant: str = "s0",flagouts4=False) -> nn.Module:
    """Get MobileOne model.
    ...
    return MobileOne(num_classes=num_classes, inference_mode=inference_mode,flagouts4=flagouts4,
                     **variant_params)
...
def reparameterize_model_allskipscale(model: torch.nn.Module) -> nn.Module:
    """ Method returns a model where a multi-branched structure
        used in training is re-parameterized into a single branch
        for inference.
...
if __name__=="__main__":
    model = mobileone(variant='s0',flagouts4=True)
    # ... train ...

    # Load Pre-trained checkpoint for fine-tuning
    checkpoint = torch.load('/home/ethan/Codes/ml-mobileone-main/mobileone_s0_unfused.pth.tar')
    ...
    model = reparameterize_model_train(model)
    model1 = reparameterize_models2(model1)
