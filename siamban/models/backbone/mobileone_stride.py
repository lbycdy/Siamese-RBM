...
def reparameterize_model_all(model: torch.nn.Module) -> nn.Module:
    """ Method returns a model where a multi-branched structure
        used in training is re-parameterized into a single branch
        for inference.
...
if __name__=="__main__":
    ...
    param = model_eval.state_dict()
    for key in param:
        if "running_mean" in key:
            k1 = key.replace('running_mean','running_var')
            print(key,param[key].min(),param[key].max(),param[key].mean(),k1,param[k1].min(),param[k1].max(),param[k1].mean())
    exit()
    model_eval = reparameterize_model_all(model_eval)
    ...
    def saveHist(data, name, save_dir):
        fig = plt.figure()
        plt.hist(data, 256)
    ...
    param = model_eval.state_dict()
    savedir="mobileoneorg"
    if not os.path.exists(savedir):
        os.makedirs(savedir)
    for key in param:
        w = param[key].data.numpy().reshape((-1, 1))
        saveHist(w, key, savedir)
