...
class BANDataset(Dataset):
    def __init__(self,world_size=None,batchsize=None):
        ...
        for name in cfg.DATASET.NAMES:
            ...
            info3 = "{} {} datalist_other;{} datalist_otherper".format(name,len(self.datalist_other),len(self.datalist_otherper))
            logger.info(info1)
            logger.info(info2)
            logger.info(info3)
    ...
    def __getitem__(self, index):
        ...
        for i in range(6):
            imgs_per_temp1.append(imgs_per_temp1[i])
            boxes_per1.append(boxes_per1[i])
            ...
            if dsettag=="remoimg":
                ...
                if flagcroptemplate:
                    I_prev, imgs_curr, bboxes_gt, _ = self.MakeExamplesCropTemplate(
                        ...
                        img_prev, img_curr, bbox_prev, bbox_curr,imgs_per1,boxes_per1,boxesother=boxes_other)
                    idxtemplate = idx
                else:

                    I_prev, imgs_curr, bboxes_gt, _ = self.MakeExamples(cfg.DATASET.CATDOGHORSETK.NUM_GENERATE_PER_IMAGE,img_prev, img_curr, bbox_prev, bbox_curr,imgs_per,boxes_per,boxesother=boxes_other)
                ...
                I_prev, imgs_curr, bboxes_gt, _ = self.MakeExamples(cfg.DATASET.CATDOGHORSETK.NUM_GENERATE_PER_FRAME,
                                                                    img_prev, img_curr, bbox_prev, bbox_curr,imgs_per,boxes_per)
                imgs_prev_temp.append(I_prev)
                imgs_curr_all.extend(imgs_curr)
                boxes_gt_all.extend(bboxes_gt)
                label_posnegpair.extend([1] * len(imgs_curr))
    ...
    def MakeExamplesCropTemplate(self, num_generated_examples, image_prev, image_curr, bbox_prev, bbox_curr,imgs_per,boxes_per, seggt=None,boxesother=[]):
        imgs_curr = []
        bboxes_gt = []
        seggt_list = []
        I_prev,bbox_fake = self.getcropedprevimg(image_prev, bbox_prev)
        I_currs, box_gts, segs = self.MakeTrainingExamplesCropedTemplate(num_generated_examples, bbox_fake,image_curr, bbox_curr,imgs_per,boxes_per, seggt,boxesother)
        imgs_curr.extend(I_currs)
        bboxes_gt.extend(box_gts)
        seggt_list.extend(segs)
        return I_prev, imgs_curr, bboxes_gt, seggt_list

    def MakeExamples(self, num_generated_examples, image_prev, image_curr, bbox_prev, bbox_curr,imgs_per,boxes_per,seggt=None,boxesother=[]):
        imgs_curr = []
        bboxes_gt = []
        seggt_list = []
        ...
        I_currs, box_gts,segs = self.MakeTrainingExamples1(num_generated_examples - 1, image_curr, bbox_curr,imgs_per,boxes_per,seggt,boxesother)
        imgs_curr.extend(I_currs)
        bboxes_gt.extend(box_gts)
        seggt_list.extend(segs)
        return I_prev, imgs_curr, bboxes_gt, seggt_list
    ...
    def MakeTrainingExamples1(self, num_generated_examples, image_curr, bbox_curr,imgs_per,boxes_per,seggt=None,boxesother=[]):
        imgs_curr = []
        bboxes_gt = []
        seg_crop = []
        ...
        for i in range(num_generated_examples):
            ...
            if np.random.random() < cfg.DATASET.CATDOGHORSETK.PERMATTING and imgs_per[i].shape[0] != 0 and imgs_per[i].shape[1] != 0:
                for k in range(100):
                    ...
                    if self.IoU(boxes_matting,boxes_per_temp[i % len(imgs_per)]) > 0 and self.IoU(boxes_matting,boxes_per_temp[i % len(imgs_per)]) <= 0.3:
    ...
    def MakeTrainingExamples(self, num_generated_examples, image_curr, bbox_curr,seggt=None,boxesother=[]):
        ...
        if seggt is not None:
            ...
            return imgs_curr, bboxes_gt, [None, ]
    def MakeTrainingExamplesCropedTemplate(self, num_generated_examples,bbox_fake,image_curr, bbox_curr,imgs_per,boxes_per,seggt=None,boxesother=[]):
        imgs_curr = []
        bboxes_gt = []
        seg_crop = []
        w, h, x_center, y_center = 0, 0, 0, 0
