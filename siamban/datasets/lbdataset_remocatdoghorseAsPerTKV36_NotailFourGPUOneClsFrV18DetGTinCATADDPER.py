...
class BANDataset(Dataset):
    ...
    def MakeExamplesCropTemplate(self, num_generated_examples, image_prev, image_curr, bbox_prev, bbox_curr, seggt=None,boxesother=[]):
        imgs_curr = []
        bboxes_gt = []
        seggt_list = []
        I_prev,bbox_fake = self.getcropedprevimg(image_prev, bbox_prev)
        I_currs, box_gts, segs = self.MakeTrainingExamplesCropedTemplate(num_generated_examples, bbox_fake,image_curr, bbox_curr, seggt,boxesother)
        ...
        for i in range(num_generated_examples):
            ...
            if np.random.random() < cfg.DATASET.CATDOGHORSETK.PERMATTING and imgs_per[i].shape[0] != 0 and imgs_per[i].shape[1] != 0:
                ...
                for k in range(50):
                    ...
                    if self.IoU(boxes_matting, boxes_per_temp[i % len(imgs_per)]) > 0 and self.IoU(boxes_matting,
                                                                                                   boxes_per_temp[
                                                                                                       i % len(imgs_per)]) <= 0.3:
    ...
    def MakeTrainingExamples(self, num_generated_examples, image_curr, bbox_curr,seggt=None,boxesother=[]):
        ...
        if seggt is not None:
            ...
            return imgs_curr, bboxes_gt, [None, ]
    def MakeTrainingExamplesCropedTemplate(self, num_generated_examples,bbox_fake,image_curr, bbox_curr,seggt=None,boxesother=[]):
        imgs_curr = []
        bboxes_gt = []
        seg_crop = []
        w, h, x_center, y_center = 0, 0, 0, 0
