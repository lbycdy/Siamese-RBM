...
class AnchorTarget1:
    ...
    def __call__(self, target, size, neg=False):
        ...
        def select(position, keep_num=16):
            print('###########2222',position[0].shape)
            print('###########2223', position[1].shape)
            print('###########2224', position[2].shape)
            exit()
            num = position[0].shape[0]
            if num <= keep_num:
                return position, num
...
class BANDataset(Dataset):
    def __init__(self,world_size=None,batchsize=None):
        ...
        self.anchor_target1 = AnchorTarget1()
    ...
    def __getitem__(self, index):
        ...
        for i in range(len(label_posnegpair)):
            ...
            if cfg.DATASET.CATDOGHORSETK.CROPBB:
                ...
                # print(x1,y1,x2,y2)
                if cfg.DATASET.CATDOGHORSETK.ANCHOR_MODE == 0:
                    _, delta, delta_weight, overlap = self.anchor_target(bbox, cfg.TRAIN.OUTPUT_SIZE, flagneg)
                    cls, _, _, _ = self.anchor_target(bbox_crop, cfg.TRAIN.OUTPUT_SIZE, flagneg)
                elif cfg.DATASET.CATDOGHORSETK.ANCHOR_MODE == 1:
                    _, delta = self.anchor_target1(bbox, cfg.TRAIN.OUTPUT_SIZE, flagneg)
                    cls, _ = self.anchor_target1(bbox_crop, cfg.TRAIN.OUTPUT_SIZE, flagneg)
            else:
                bbox = Corner(x1, y1, x2, y2)
                searchboxes.append([x1, y1, x2, y2])
