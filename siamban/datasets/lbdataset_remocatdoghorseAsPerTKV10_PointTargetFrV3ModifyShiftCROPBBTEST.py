...
class BANDataset(Dataset):
    def __init__(self,world_size=None,batchsize=None):
        ...
        for name in cfg.DATASET.NAMES:
            ...
            if name in ["HeadNOFACECOCO","HeadNOFACEAIC"]:
                ...
                print(info1)
                logger.info(info1)
            elif name in ["REMOCATDOGHORSE","REMOWIKICATHIGDOGANNORESULT20230324","REMOWIKICATHIGDOGANNORESULT20230328","REMOWIKICATHIGDOGANNORESULT20230331","REMOWIKICATHIGDOGANNORESULT20230407"]:
                for imgpath in data:
                ...
                for video in data:
                    for track in data[video]:
                        ...
                        if c in self.clstag2clsid_dict:
                            if name in ['COCO', 'DET']:
                                boxinfo = ['got', rootid, imgpath_list[0], box_list[0]]
                            else:
            ...
            if name in ['REMOCATDOGHORSE', 'REMOWIKICATHIGDOGANNORESULT20230328','REMOWIKICATHIGDOGANNORESULT20230331', 'REMOWIKICATHIGDOGANNORESULT20230407','REMOWIKICATHIGDOGANNORESULT20230324', 'COCO', 'DET']:
                if len(cat_list) > 0:
    ...
    def __getitem__(self, index):
        ...
        if random.uniform(0,1.0)<0.7:
            ...
            if random.uniform(0,1.0)<cfg.DATASET.CATDOGHORSETK.PROB_CROPTEMPLATE:
                ...
                sx = random.uniform(cfg.DATASET.CATDOGHORSETK.SCALE_CROPTEMPLATE,0.85)
                sy = random.uniform(cfg.DATASET.CATDOGHORSETK.SCALE_CROPTEMPLATE,0.85)
            ...
            for i in range(cfg.DATASET.CATDOGHORSETK.NUM_NEG_OTHEROBJECT):
                if random.uniform(0, 1.0) < cfg.DATASET.CATDOGHORSETK.PROB_NEGFROM_HEADNOFACE:
                    ...
                    imageother, box_other = self.get_otherobj()
                I_currs, box_gts, _ = self.MakeTrainingExamples1(1, imageother, box_other, box_other, [0, 0, 160, 160])
                imgs_curr_all.append(I_currs[0])
                boxes_gt_all.append(box_gts[0])
                label_posnegpair.append(0)
            if random.uniform(0, 1.0) < cfg.DATASET.CATDOGHORSETK.PROB_IMG:
                ...
                img_curr = img_prev.copy()
                bbox_curr = bbox_prev[:]
                I_prev, imgs_curr, bboxes_gt, _ = self.MakeExamples1(cfg.DATASET.CATDOGHORSETK.NUM_GENERATE_PER_IMAGE,
                                                                    img_prev, img_curr, bbox_prev, bbox_curr)
                imgs_curr_all.extend(imgs_curr)
                boxes_gt_all.extend(bboxes_gt)
                label_posnegpair.extend([1] * len(imgs_curr))
                ...
                if dsettag == 'got':
                    bbox_prev = self._get_bbox(img_prev, bbox_prev)
                    bbox_curr = self._get_bbox(img_curr, bbox_curr)
                I_prev, imgs_curr, bboxes_gt, _ = self.MakeExamples1(cfg.DATASET.CATDOGHORSETK.NUM_GENERATE_PER_FRAME,
                                                                    img_prev, img_curr, bbox_prev, bbox_curr)
                imgs_curr_all.extend(imgs_curr)
                boxes_gt_all.extend(bboxes_gt)
                label_posnegpair.extend([1] * len(imgs_curr))
    ...
    def MakeExamples(self, num_generated_examples, image_prev, image_curr, bbox_prev, bbox_curr,seggt=None):
        ...
        bboxes_gt = []
        seggt_list = []
        I_prev,_ = self.getprevimg(image_prev, bbox_prev)
    ...
    def MakeExamples1(self, num_generated_examples, image_prev, image_curr, bbox_prev, bbox_curr,seggt=None):
        ...
        bboxes_gt = []
        seggt_list = []
        I_prev_crop = [0,0,160,160]
        I_prev,I_prev_box = self.getprevimg(image_prev, bbox_prev)
        ...
        if random.uniform(0,1.0)<cfg.DATASET.CATDOGHORSETK.PROB_CROPTEMPLATE:
            ...
            I_prev = I_prev[ymin:ymax,xmin:xmax,:]
            I_prev = cv2.resize(I_prev,(self.resizedw_temp,self.resizedw_temp))
            I_prev_crop = [xmin,ymin,xmax,ymax]

        I_curr, box_gt,seg = self.MakeTrueExample1(image_curr, bbox_prev, bbox_curr,I_prev_box,I_prev_crop,seggt)
        imgs_curr.append(I_curr)
        bboxes_gt.append(box_gt)
        seggt_list.append(seg)
        I_currs, box_gts,segs = self.MakeTrainingExamples1(num_generated_examples - 1, image_curr, bbox_curr,I_prev_box,I_prev_crop,seggt)
        imgs_curr.extend(I_currs)
        bboxes_gt.extend(box_gts)
        seggt_list.extend(segs)
        return I_prev, imgs_curr, bboxes_gt, seggt_list
    ...
    def MakeTrainingExamples1(self, num_generated_examples, image_curr, bbox_curr,I_prev_box,I_prev_crop,seggt=None):
        imgs_curr = []
        bboxes_gt = []
        seg_crop = []
        w, h, x_center, y_center = 0, 0, 0, 0
        for i in range(num_generated_examples):
            crop_box = self.shift_bbox_remo9(bbox_curr,I_prev_box)
            xmin, ymin, xmax, ymax = crop_box
            x_center = (xmin + xmax) / 2
            y_center = (ymin + ymax) / 2
            w = xmax - xmin
            ...
            roi_box = [roi_x1, roi_y1, roi_x2, roi_y2]
            I_curr = self.getcropedimage(image_curr, roi_box,self.resizedw_search,self.resizedh_search)
            I_curr = I_curr[I_prev_crop[1]:I_prev_crop[3], I_prev_crop[0]:I_prev_crop[2], :]
            I_curr = cv2.resize(I_curr, (160, 160))
            imgs_curr.append(I_curr)
            if seggt is not None:
                seg_curr = self.getcropedimage(seggt, roi_box,self.resizedw_search,self.resizedh_search)
                seg_crop.append(seg_curr)
            ...
            gt_x1 -= I_prev_crop[0] / 160
            gt_x2 -= I_prev_crop[0] / 160
            gt_y1 -= I_prev_crop[1] / 160
            gt_y2 -= I_prev_crop[1] / 160
            gt_x1 = gt_x1 / (I_prev_crop[2] / 160 - I_prev_crop[0] / 160 + 1e-8)
            gt_x2 = gt_x2 / (I_prev_crop[2] / 160 - I_prev_crop[0] / 160 + 1e-8)
            gt_y1 = gt_y1 / (I_prev_crop[3] / 160 - I_prev_crop[1] / 160 + 1e-8)
            gt_y2 = gt_y2 / (I_prev_crop[3] / 160 - I_prev_crop[1] / 160 + 1e-8)
            gt_x1 *= self.ScaleFactor
            gt_y1 *= self.ScaleFactor
            gt_x2 *= self.ScaleFactor
            gt_y2 *= self.ScaleFactor
    ...
    def MakeTrueExample1(self, image_curr, bbox_prev, bbox_curr,I_prev_box,I_prev_crop,seggt=None):
        ...
        #     w = max(w, h)
        #     h = w
        w = I_prev_box[2] - I_prev_box[0]
        h = I_prev_box[3] - I_prev_box[1]
        scale = float(self.resizedw_search)/float(self.resizedw_temp)
        w *= scale
        h *= scale
        roi_x1 = x_center - w / 2.0
        ...
        img_curr = img_curr[I_prev_crop[1]:I_prev_crop[3],I_prev_crop[0]:I_prev_crop[2], : ]
        img_curr = cv2.resize(img_curr,(160,160))
        gt_x1, gt_y1, gt_x2, gt_y2 = bbox_curr
        gt_x1 -= roi_x1
        gt_x2 -= roi_x1
        gt_y1 -= roi_y1
    ...
    def shift_bbox_remo9(self, box,I_prev_box):  # box = [xmin,ymin,xmax,ymax]
        """
        ...
        cp_w = I_prev_box[2] - I_prev_box[0]
        cp_h = I_prev_box[3] - I_prev_box[1]
        cx_box = (xmin + xmax) / 2
        cy_box = (ymax + ymin) / 2
