...
class BANDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        for i in range(cfg.DATASET.CATDOGHORSETK.NUM_NEG_OTHEROBJECT):
            ...
            I_currs, box_gts,_ = self.MakeTrainingExamples(1, imageother, box_other,box_other,[0,0,160,160])
    ...
    def MakeExamples(self, num_generated_examples, image_prev, image_curr, bbox_prev, bbox_curr,seggt=None):
        ...
        I_curr, box_gt,seg = self.MakeTrueExample(image_curr, bbox_prev, bbox_curr,I_prev_box,I_prev_crop,seggt)
        imgs_curr.append(I_curr)
        bboxes_gt.append(box_gt)
        seggt_list.append(seg)
        I_currs, box_gts,segs = self.MakeTrainingExamples(num_generated_examples - 1, image_curr, bbox_curr,I_prev_box,I_prev_crop,seggt)
    ...
    def MakeTrainingExamples(self, num_generated_examples, image_curr, bbox_curr,I_prev_box,I_prev_crop,seggt=None):
        imgs_curr = []
        bboxes_gt = []
        seg_crop = []
    ...
    def MakeTrueExample(self, image_curr, bbox_prev, bbox_curr,I_prev_box,I_prev_crop,seggt=None):
        ...
        gt_y1 -= I_prev_crop[1]
        gt_y2 -= I_prev_crop[1]
        gt_x1 = gt_x1 / (I_prev_crop[2] - I_prev_crop[0] + 1e-8)
        gt_x2 = gt_x2 / (I_prev_crop[2] - I_prev_crop[0] + 1e-8)
        gt_y1 = gt_y1 / (I_prev_crop[3] - I_prev_crop[1] + 1e-8)
        gt_y2 = gt_y2 / (I_prev_crop[3] - I_prev_crop[1] + 1e-8)
        gt_x1 *= self.ScaleFactor
        gt_y1 *= self.ScaleFactor
        gt_x2 *= self.ScaleFactor
        gt_y2 *= self.ScaleFactor
