...
class BANDataset(Dataset):
    def __init__(self,world_size=None,batchsize=None):
        ...
        # create point target
        if self.comparetime(cfg.TRAIN.CODE_TIME_TAGE, 'T20221218'):
    ...
    def __getitem__(self, index):
        ...
        if random.uniform(0,1.0)<cfg.DATASET.CATDOGHORSETK.PROB_CROPTEMPLATE:
            ...
            if self.comparetime(cfg.TRAIN.CODE_TIME_TAGE,'T20221213'):
                ...
                if w_target<sizehalf:
                    xmin = 20+random.randint(0,sizehalf - w_target-1)
                else:
                    xmin = 20
                if h_target < sizehalf:
                    ymin = 20 + random.randint(0, sizehalf - h_target - 1)
                else:
                    ymin = 20
            xmax = xmin + w_target
            ymax = ymin + h_target
            if self.comparetime(cfg.TRAIN.CODE_TIME_TAGE,'T20221227'):
                w4 = w_target // 2
                h4 = h_target // 2
            else:
                w4 = w_target//4
        ...
        if cfg.DATASET.PERSONTK.FLAG_ENCODEGTASDET:
            prior_width = 1.0
            prior_height = 1.0
            ...
            prior_center_y = 0.5
            boxes_gt_all_new = []
            for i_batch in range(len(boxes_gt_all)):
                x1, y1, x2, y2 = boxes_gt_all[i_batch]
                bbox_center_x = (x1 + x2) / 2.0
                bbox_center_y = (y1 + y2) / 2.0
                bbox_width = (x2 - x1)
                bbox_height = (y2 - y1)
                x1 = (bbox_center_x - prior_center_x) / prior_width / prior_variance[0]
                y1 = (bbox_center_y - prior_center_y) / prior_height / prior_variance[1]
                x2 = math.log(bbox_width / prior_width + 1e-8) / prior_variance[2]
                y2 = math.log(bbox_height / prior_height + 1e-8) / prior_variance[3]
                boxes_gt_all_new.append([x1, y1, x2, y2])

        delta1 = np.stack(boxes_gt_all_new)
    ...
    def shift_bbox_remo(self, box):  # box = [xmin,ymin,xmax,ymax]
        ...
        if self.comparetime(cfg.TRAIN.CODE_TIME_TAGE, 'T20221218'):
            cp_w = (xmax - xmin) * 2 * float(self.resizedw_search)/float(self.resizedw_temp)
            cp_h = (ymax - ymin) * 2 * float(self.resizedw_search)/float(self.resizedw_temp)
        else:
            cp_w = (xmax - xmin) * 2
            cp_h = (ymax - ymin) * 2
        cx_box = (xmin + xmax) / 2
        ...
        for i in range(20):
            ...
            x2 = x1 + cp_w
            y2 = y1 + cp_h
            if self.coveA(box, [x1, y1, x2, y2]) > cfg.DATASET.CATDOGHORSETK.COVEA:
                flagok = True
                break
        if not flagok:
            x1 = cx_box - cp_w / 2
    ...
    def shift_bbox_remo2(self, box):  # box = [xmin,ymin,xmax,ymax]
        ...
        r_rand = cfg.DATASET.CATDOGHORSETK.LAMBDA_MIN_RATIO + random.uniform(0, 1.0) * (cfg.DATASET.CATDOGHORSETK.LAMBDA_MAX_RATIO - cfg.DATASET.CATDOGHORSETK.LAMBDA_MIN_RATIO)
        cp_w *= (1.0 + r_rand)
        cp_h /= (1.0 + r_rand)
        cp_w *= (1.0 + s_rand)
        cp_h *= (1.0 + s_rand)
