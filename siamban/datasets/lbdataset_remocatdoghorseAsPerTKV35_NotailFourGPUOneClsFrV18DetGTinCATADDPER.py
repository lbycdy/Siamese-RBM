...
class PointTarget3:
    ...
    def __call__(self, target, size, neg=False):
        ...
        for i in range(len(target)):
            ...
            flagpos = np.square(tcx - points[0]) / np.square(tw / cfg.POINT.SCALE_POS) + np.square(
                tcy - points[1]) / np.square(th / cfg.POINT.SCALE_POS) < 1
            flagneg = np.square(tcx - points[0]) / np.square(tw / cfg.POINT.SCALE_NEG) + np.square(
                tcy - points[1]) / np.square(th / cfg.POINT.SCALE_NEG) > 1
...
class BANDataset(Dataset):
    def __init__(self, world_size=None, batchsize=None):
        ...
        for name in cfg.DATASET.NAMES:
            ...
            if name in ["HeadNOFACECOCO", "HeadNOFACEAIC"]:
                ...
                print(info1)
                logger.info(info1)
            elif name in ["REMANNONOTAILDATA230525230526IMAGE", "REMANNONOTAILDATA230525230613IMAGE",
                ...
                for imgpath in data:
                    for box in data[imgpath]:
                        ...
                        boxinfo = ['AIC', rootid, imgpath, [xmin, ymin, xmax, ymax]]
                        # print(boxinfo)
            ...
            if name in ["REMANNONOTAILDATA230525230526IMAGE", "REMANNONOTAILDATA230525230613IMAGE",
                        "REMANNONOTAILDATA0616IMAGE"]:
                if len(cat_list) > 0:
            ...
            info1 = "{} {} catimg; {} dogimg; {} horseimg.".format(name, len(self.catlist_img), len(self.doglist_img),
                                                                   len(self.horselist_img))
            info2 = "{} {} catvid; {} dogvid; {} horsevid.".format(name, len(self.catlist_vid), len(self.doglist_vid),
                                                                   len(self.horselist_vid))
            info3 = "{} {} datalist_other;{} datalist_otherper".format(name, len(self.datalist_other),
                                                                       len(self.datalist_otherper))
            logger.info(info1)
            logger.info(info2)
            logger.info(info3)
    ...
    def __getitem__(self, index):
        ...
        if cfg.DATASET.CATDOGHORSETK.NUM_NEG_OTHERPER > 0:
            for di in self.datalist_per[index % self.videos_per_epoch]:
                imageother, box_other = self.get_otherobj1(di)
                I_currs, box_gts, _ = self.MakeTrainingExamples(1, imageother, box_other)
                imgs_curr_all.append(I_currs[0])
                ...
                label_posnegpair.append(0)
        for i in range(5):
            imgs_per_temp.append(imgs_per_temp[i])
            boxes_per.append(boxes_per[i])
        for i in range(len(imgs_per_temp)):
            imgper1 = imgs_per_temp[i][boxes_per[i][1]:boxes_per[i][3], boxes_per[i][0]:boxes_per[i][2]]
            imgs_per.append(imgper1)
        ...
        for idx, dinfo in enumerate(self.datalist_pos[index % self.videos_per_epoch]):
            ...
            if dsettag == "remoimg":
                ...
                flagcroptemplate = (cid == 1 and p < cfg.DATASET.CATDOGHORSETK.PROB_CROPTEMPLATE_CAT) or (
                            cid != 1 and p < cfg.DATASET.CATDOGHORSETK.PROB_CROPTEMPLATE_OTHER)
                if flagcroptemplate:
                    I_prev, imgs_curr, bboxes_gt, _ = self.MakeExamplesCropTemplate(
                        cfg.DATASET.CATDOGHORSETK.NUM_GENERATE_PER_IMAGE,
                        img_prev, img_curr, bbox_prev, bbox_curr,boxesother=boxes_other)
                    idxtemplate = idx
                else:
                    I_prev, imgs_curr, bboxes_gt, _ = self.MakeExamples(cfg.DATASET.CATDOGHORSETK.NUM_GENERATE_PER_IMAGE,
                                                                        img_prev, img_curr, bbox_prev, bbox_curr, imgs_per,
                                                                        boxes_per, boxesother=boxes_other)
    ...
    def MakeExamplesCropTemplate(self, num_generated_examples, image_prev, image_curr, bbox_prev, bbox_curr, seggt=None,
                                 boxesother=[]):
        imgs_curr = []
        bboxes_gt = []
        seggt_list = []
        I_prev, bbox_fake = self.getcropedprevimg(image_prev, bbox_prev)
        I_currs, box_gts, segs = self.MakeTrainingExamplesCropedTemplate(num_generated_examples, bbox_fake, image_curr,
                                                                         bbox_curr, seggt, boxesother)
        imgs_curr.extend(I_currs)
        bboxes_gt.extend(box_gts)
        seggt_list.extend(segs)
        return I_prev, imgs_curr, bboxes_gt, seggt_list

    def MakeExamples(self, num_generated_examples, image_prev, image_curr, bbox_prev, bbox_curr, imgs_per, boxes_per,
                     seggt=None, boxesother=[]):
        imgs_curr = []
        bboxes_gt = []
        seggt_list = []
        ...
        I_currs, box_gts, segs = self.MakeTrainingExamples1(num_generated_examples - 1, image_curr, bbox_curr, imgs_per,
                                                            boxes_per, seggt, boxesother)
        imgs_curr.extend(I_currs)
        bboxes_gt.extend(box_gts)
        seggt_list.extend(segs)
        return I_prev, imgs_curr, bboxes_gt, seggt_list
    ...
    def MakeTrainingExamples1(self, num_generated_examples, image_curr, bbox_curr, imgs_per, boxes_per, seggt=None,
                              boxesother=[]):
        imgs_curr = []
        bboxes_gt = []
        seg_crop = []
        ...
        for i in range(num_generated_examples):
            ...
            # print(boxes_per)
            if boxes_per[i][3] - boxes_per[i][1] > 10 and boxes_per[i][2] - boxes_per[i][0] > 10:
                imageper = imgs_per[i]
                boxes_matting = [int(gt_x1 * 160), int(gt_y1 * 160),
                                 int(gt_x2 * 160), int(gt_y2 * 160)]

                w_per1 = boxes_per[i][2] - boxes_per[i][0]
                h_per1 = boxes_per[i][3] - boxes_per[i][1]
                r = h_per1 / w_per1
                w_matting = boxes_matting[2] - boxes_matting[0]
                c = random.randint(int(w_matting * 0.8), int(w_matting * 1.2))
                d = c * r
                a = random.random()
                b = random.random()
                boxes_per_temp = boxes_per
                boxes_per_temp[i][1] = a * 160
                boxes_per_temp[i][0] = b * 160
                ...
                if self.IoU(boxes_matting,boxes_per[i%len(imgs_per)])>0.05 and self.IoU(boxes_matting,boxes_per[i%len(imgs_per)])<0.5:
                    ...
                    I_curr[boxes_per_temp[i][1]:boxes_per_temp[i][3], boxes_per_temp[i][0]:boxes_per_temp[i][2],:] = imageper
                    I_curr[boxes_matting[1]:boxes_matting[3], boxes_matting[0]:boxes_matting[2]] = img_curr_matting1
            imgs_curr.append(I_curr)
    ...
    def MakeTrainingExamplesCropedTemplate(self, num_generated_examples, bbox_fake, image_curr, bbox_curr, seggt=None,
                                           boxesother=[]):
        imgs_curr = []
        bboxes_gt = []
        seg_crop = []
    ...
    def getcropedimage1(self, image, bbox, targetw, targeth):
        imgdim = image.shape
        imgh, imgw = imgdim[:2]
        x1, y1, x2, y2 = bbox
