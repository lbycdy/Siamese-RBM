...
class BANDataset(Dataset):
    def __init__(self,rank,world_size=None,batchsize=None):
        ...
        for name in cfg.DATASET.NAMES:
            ...
            if name in ["HeadNOFACECOCO","HeadNOFACEAIC"]:
                ...
                print(info1)
                logger.info(info1)
            elif name in ["REMOWIKICATHIGDOGANNORESULT20230427", "REMOWIKICATHIGDOGANNORESULT20230504","REMOWIKICATHIGDOGANNORESULT20230508","REMOWIKICATHIGDOGANNORESULT20230525","REMOWIKICATHIGDOGANNORESULT20230526"]:
                for imgpath in data:
                    imgname = os.path.basename(imgpath)
                    if "C_" != imgname[:2] and "CD" != imgname[:2] and "D_" != imgname[:2]:
                        for box in data[imgpath]:
                            ...
                            if cid == 1:
                                ...
                                cat_img_list.append(boxinfo)
                            elif cid == 2:
                                dog_list.append(boxinfo)
                                dog_img_list.append(boxinfo)
                            elif cid == 3:
                                horse_list.append(boxinfo)
                                horse_img_list.append(boxinfo)

            elif name == 'REMOWIKICATHIGDOGANNORESULTVID202305227':
                for video in data:
                    for track in data[video]:
                        imgpath_list = []
                        box_list = []
                        for frames in data[video][track]:
                            # print(frames)
                            box = data[video][track][frames]
                            # print(box)
                            imgpath = os.path.join(video,frames)
                            imgpath_list.append(imgpath)
                            box_list.append(box)
                        boxinfo = [['got', rootid, imgpath_list, box_list,1]]
                        cat_list.append(boxinfo)
    ...
    def __getitem__(self, index):
        ...
        for i in range(cfg.DATASET.CATDOGHORSETK.NBATCH_SHAPREPREV):
            if random.uniform(0,1.0)<cfg.DATASET.CATDOGHORSETK.PROB_IMG:
                ...
                if indret == 1:
                    b=random.randint(0,self.num_cat_img-4)
                    dsettag, rootid, imgpath, bbox_prev, _ = self.catlist_img[b]
                elif indret == 2:
                    b=random.randint(0,self.num_dog_img-4)
                    dsettag, rootid, imgpath, bbox_prev, _ = self.doglist_img[b]
                elif indret == 3:
                    b=random.randint(0,self.num_horse_img-4)
                    dsettag, rootid, imgpath, bbox_prev, _ = self.horselist_img[b]
                ...
                if indret == 1:
                    b=random.randint(0,self.num_cat_vid-4)
                    dsettag, rootid, imgpath_list, bbox_list, _ = self.catlist_vid[b]
                elif indret == 2:
                    b=random.randint(0,self.num_dog_vid-4)
                    dsettag, rootid, imgpath_list, bbox_list, _ = self.doglist_vid[b]
                elif indret == 3:
                    b=random.randint(0,self.num_horse_vid-4)
                    dsettag, rootid, imgpath_list, bbox_list, _ = self.horselist_vid[b]
                ...
                if dsettag == 'got':
                    ...
                    bbox_prev = bbox_prev
                    bbox_curr = bbox_curr
                I_prev, imgs_curr, bboxes_gt,_ = self.MakeExamples(cfg.DATASET.CATDOGHORSETK.NUM_GENERATE_PER_FRAME,img_prev, img_curr, bbox_prev, bbox_curr)
                imgs_prev_temp.append(I_prev)
                imgs_curr_all.extend(imgs_curr)
                boxes_gt_all.extend(bboxes_gt)
                label_posnegpair.extend([1]*len(imgs_curr))
