...
class BANDataset(Dataset):
    def __init__(self,world_size=None,batchsize=None):
        ...
        for name in cfg.DATASET.NAMES:
            ...
            if name in ["HeadNOFACECOCO","HeadNOFACEAIC"]:
                ...
                print(info1)
                logger.info(info1)
            elif name in ["REMOCATDOGHORSE","REMOWIKICATHIGDOGANNORESULT20230324","REMOWIKICATHIGDOGANNORESULT20230328","REMOWIKICATHIGDOGANNORESULT20230331","REMOWIKICATHIGDOGANNORESULT20230407","REMOWIKICATHIGDOGANNORESULT20230427"]:
                for imgpath in data:
            ...
            if name in ['REMOCATDOGHORSE', 'REMOWIKICATHIGDOGANNORESULT20230328','REMOWIKICATHIGDOGANNORESULT20230331', 'REMOWIKICATHIGDOGANNORESULT20230407','REMOWIKICATHIGDOGANNORESULT20230324', 'COCO', 'DET','REMOWIKICATHIGDOGANNORESULT20230427']:
                if len(cat_list) > 0:
    ...
    def __getitem__(self, index):
        ...
        for i in range(cfg.DATASET.CATDOGHORSETK.NBATCH_SHAPREPREV):
            if random.uniform(0,1.0)<cfg.DATASET.CATDOGHORSETK.PROB_IMG:
                ...
                if random.uniform(0.0, 1.0) < cfg.DATASET.CATDOGHORSETK.MAKEEAAMPLES1PROB:
                    ...
                    imgs_prev_all.extend(I_prev_list)
                    imgs_curr_all.extend(imgs_curr * len(I_prev_list))
                    boxes_gt_all.extend(bboxes_gt * len(I_prev_list))
                    ...
                    imgs_prev_all.extend([I_prev] * len(imgs_curr))
                ...
                if random.uniform(0.0, 1.0) < cfg.DATASET.CATDOGHORSETK.MAKEEAAMPLES1PROB:
                    I_prev_list, imgs_curr, bboxes_gt, _ = self.MakeExamples1(
                        cfg.DATASET.CATDOGHORSETK.NUM_GENERATE_PER_IMAGE, img_prev, img_curr, bbox_prev, bbox_curr)
                    imgs_prev_all.extend(I_prev_list)
                    imgs_curr_all.extend(imgs_curr * len(I_prev_list))
                    boxes_gt_all.extend(bboxes_gt * len(I_prev_list))
                    label_posnegpair.extend([1] * len(I_prev_list))
                    ...
                    I_prev, imgs_curr, bboxes_gt, _ = self.MakeExamples(
                        cfg.DATASET.CATDOGHORSETK.NUM_GENERATE_PER_IMAGE, img_prev, img_curr, bbox_prev, bbox_curr)
                    imgs_prev_temp.append(I_prev)
                    imgs_prev_all.extend([I_prev] * len(imgs_curr))
                    imgs_curr_all.extend(imgs_curr)
        ...
        if MAKEEAAMPLES1FLAG == 0:
            imgs_prev_all.extend([I_prev_list[random.randint(0,11)]] * cfg.DATASET.CATDOGHORSETK.NUM_NEG_OTHEROBJECT)
        elif MAKEEAAMPLES1FLAG == 1:
            I_prev_num = np.random.choice((0, 1, 2, 3))
            I_prev = imgs_prev_temp[0]
            ...
            imgs_prev_all.extend([I_prev] * cfg.DATASET.CATDOGHORSETK.NUM_NEG_OTHEROBJECT)
        ###############################finish get data
        ...
        ntemplate = len(imgs_prev_all)
        for i in range(ntemplate):
            # print('##############',imgs_prev_all[i].shape)
            imgs_prev_all[i] = Distortion(imgs_prev_all[i])
            if cfg.DATASET.CATDOGHORSETK.FLIP_TEMPLATE and cfg.DATASET.CATDOGHORSETK.FLIP_TEMPLATE > np.random.random():
                imgs_prev_all[i] = cv2.flip(imgs_prev_all[i], 1)
            if cfg.DATASET.CATDOGHORSETK.FLIPVER_TEMPLATE and cfg.DATASET.CATDOGHORSETK.FLIPVER_TEMPLATE > np.random.random():
                imgs_prev_all[i] = cv2.flip(imgs_prev_all[i], 0)
    ...
    def MakeExamples1(self, num_generated_examples, image_prev, image_curr, bbox_prev, bbox_curr,seggt=None):
        ...
        I_currs, box_gts, segs = self.MakeTrainingExamples(1, image_curr, bbox_curr,seggt)
        imgs_curr.extend(I_currs)
        bboxes_gt.extend(box_gts)
        seggt_list.extend(segs)
    ...
    def getprevimg(self, image_prev, bbox_prev):
        ...
        return imgcrop
    def getprevimgCrop(self, image_prev, bbox_prev, scalemin=0.6, scalemax=1.0):
        x1, y1, x2, y2 = bbox_prev
        x_center = (x1 + x2) / 2.0
        y_center = (y1 + y2) / 2.0
        w = x2 - x1
