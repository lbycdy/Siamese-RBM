...
class BANDataset(Dataset):
    def __init__(self,world_size=None,batchsize=None):
        ...
        for name in cfg.DATASET.NAMES:
            ...
            elif name in ["REMOWIKICATHIGDOGANNORESULT20230324","REMOWIKICATHIGDOGANNORESULT20230328","REMOWIKICATHIGDOGANNORESULT20230331"]:
                for imgpath in data:
                    for box in data[imgpath]:
                        xmin, ymin, xmax, ymax, cid = box
                        boxinfo = ['remo1', rootid, imgpath, [xmin, ymin, xmax, ymax]]
                        if cid == 1:
                            cat_list.append(boxinfo)
                        elif cid == 2:
                            dog_list.append(boxinfo)
            ...
            if name in ['REMOCATDOGHORSE','REMOWIKICATHIGDOGANNORESULT20230328','REMOWIKICATHIGDOGANNORESULT20230331','REMOWIKICATHIGDOGANNORESULT20230324','COCO','DET']:
                if len(cat_list) > 0:
        ...
        self.cova_thresh_shift_remo1_ = cfg.DATASET.CATDOGHORSETK.COVEA1
    ...
    def __getitem__(self, index):
        ...
        for i in range(cfg.DATASET.CATDOGHORSETK.NUM_NEG_OTHEROBJECT):
            ...
            I_currs, box_gts,_ = self.MakeTrainingExamples(1, imageother, box_other, 'remo')
        ...
        if random.uniform(0,1.0)<cfg.DATASET.CATDOGHORSETK.PROB_IMG:
            ...
            bbox_curr = bbox_prev[:]
            I_prev, imgs_curr, bboxes_gt, _ = self.MakeExamples(cfg.DATASET.CATDOGHORSETK.NUM_GENERATE_PER_IMAGE,img_prev, img_curr, bbox_prev, bbox_curr,dsettag)
            ...
            I_prev, imgs_curr, bboxes_gt,_ = self.MakeExamples(cfg.DATASET.CATDOGHORSETK.NUM_GENERATE_PER_FRAME,
                                                                img_prev, img_curr, bbox_prev, bbox_curr,dsettag)
            imgs_curr_all.extend(imgs_curr)
            boxes_gt_all.extend(bboxes_gt)
            label_posnegpair.extend([1]*len(imgs_curr))
    ...
    def MakeExamples(self, num_generated_examples, image_prev, image_curr, bbox_prev, bbox_curr, dsettag, seggt=None):
        imgs_curr = []
        bboxes_gt = []
        seggt_list = []
        ...
        I_currs, box_gts,segs = self.MakeTrainingExamples(num_generated_examples - 1, image_curr, bbox_curr, dsettag,seggt)
    ...
    def MakeTrainingExamples(self, num_generated_examples, image_curr, bbox_curr,dsettag,seggt=None):
        imgs_curr = []
        bboxes_gt = []
        seg_crop = []
        for i in range(num_generated_examples):
            if cfg.DATASET.CATDOGHORSETK.SHIFT_MODE == 0:
                ...
                crop_box = self.shift_bbox_remo7(bbox_curr,dsettag)
                xmin, ymin, xmax, ymax = crop_box
                x_center = (xmin + xmax) / 2
                y_center = (ymin + ymax) / 2
                w = xmax - xmin
    ...
    def shift_bbox_remo7(self, box,dsettag):  # box = [xmin,ymin,xmax,ymax]
        """
        ...
        for i in range(50):
            ...
            x2 = x1 + cp_w
            y2 = y1 + cp_h
            if self.coveA(box, [x1, y1, x2, y2]) > self.cova_thresh_shift_remo1_ and dsettag == 'remo1':
                # print('####################', dsettag)
                flagok = True
                break
            elif self.coveA(box, [x1, y1, x2, y2]) > self.cova_thresh_shift_:
                flagok = True
                break
        if not flagok:
            x1 = cx_box - cp_w / 2
