...
class BANDataset(Dataset):
    def __init__(self,world_size=None,batchsize=None):
        ...
        for name in cfg.DATASET.NAMES:
            ...
            if name in ["HeadNOFACECOCO","HeadNOFACEAIC"]:
            ...
            elif name in ['LASOT1', 'GOT10K1', 'VID1']:
                for vidtrackid in data:
                    ...
                    for imgname, box in imgboxinfo:
                        ...
                        box_list.append(box)
                    if catname == 'horse':
                        boxinfo = ['notail', rootid, imgpath_list, box_list, 3]
                        horse_list.append(boxinfo)
                    elif catname == 'dog':
                        boxinfo = ['notail', rootid, imgpath_list, box_list, 2]
                        dog_list.append(boxinfo)
                    elif catname == 'cat':
                        boxinfo = ['notail', rootid, imgpath_list, box_list, 1]
                        cat_list.append(boxinfo)
            else:
                ...
                for video in data:
                    for track in data[video]:
                        ...
                        if c in self.clstag2clsid_dict:
                            if name in ['COCO', 'DET']:
                                boxinfo = ['got', rootid, imgpath_list[0], box_list[0],self.clstag2clsid_dict[c]]
                            else:
            ...
            if name in ["REMOWIKICATHIGDOGANNORESULT20230427", "REMOWIKICATHIGDOGANNORESULT20230504",
                        "REMOWIKICATHIGDOGANNORESULT20230508"]:
                if len(cat_list) > 0:
            ...
            info2 = "{} {} datalist_img; {} datalist_vid;{} other_list.".format(name,len(self.datalist_img),len(self.datalist_vid),len(self.datalist_other))
            logger.info(info1)
            logger.info(info2)
            print(info1)
            print(info2)
    ...
    def __getitem__(self, index):
        ...
        #     covea = self.cova_thresh_shift_
        #     I_currs, box_gts,_ = self.MakeTrainingExamples(1, imageother, box_other,covea)
        ...
        for i in range(cfg.DATASET.CATDOGHORSETK.NBATCH_SHAPREPREV):
            if random.uniform(0,1.0)<cfg.DATASET.CATDOGHORSETK.PROB_IMG:
                ...
                bbox_curr = bbox_prev[:]
                if random.uniform(0.0, 1.0) < cfg.DATASET.CATDOGHORSETK.MAKEEAAMPLES1PROB:
                    I_prev_list, imgs_curr, bboxes_gt, _ = self.MakeExamples1(cfg.DATASET.CATDOGHORSETK.NUM_GENERATE_PER_IMAGE,img_prev, img_curr, bbox_prev, bbox_curr)
                    if MAKEEAAMPLES1FLAG == 0:
                        # imgs_prev_temp.extend([I_prev_list[0]] * cfg.DATASET.CATDOGHORSETK.NUM_NEG_OTHEROBJECT)
                        imgs_curr_all.extend(imgs_curr)
                        boxes_gt_all.extend(bboxes_gt)

                    imgs_prev_temp.extend(I_prev_list)

                    label_posnegpair.extend([1]*len(I_prev_list))
                    MAKEEAAMPLES1FLAG = 1
                    a+=1
                else:
                    ...
                    if MAKEEAAMPLES1FLAG == 0:
                        # imgs_prev_temp.extend([I_prev] * cfg.DATASET.CATDOGHORSETK.NUM_NEG_OTHEROBJECT)
                        imgs_curr_all.extend(imgs_curr)
                        boxes_gt_all.extend(bboxes_gt)
                    imgs_prev_temp.extend([I_prev] * 12)

                    label_posnegpair.extend([1]*12)
                    MAKEEAAMPLES1FLAG = 1
                    b+=1
            else:
        ...
        for i in range(len(imgs_prev_temp)):
            I_prev = imgs_prev_temp[i]
        ...
        # print('#############2',len(label_posnegpair))
        # print('#############3', len(boxes_gt_all))
        # exit()
        ...
        I_prev = np.stack(imgs_prev_all).transpose((0, 3, 1, 2))
    ...
    def MakeExamples(self, num_generated_examples, image_prev, image_curr, bbox_prev, bbox_curr,seggt=None):
        ...
        covea = self.cova_thresh_shift_
        I_currs, box_gts,segs = self.MakeTrainingExamples(4*num_generated_examples - 1, image_curr, bbox_curr,covea,seggt)
        imgs_curr.extend(I_currs)
        bboxes_gt.extend(box_gts)
        seggt_list.extend(segs)
        return I_prev, imgs_curr, bboxes_gt, seggt_list
    def MakeExamples1(self, num_generated_examples, image_prev, image_curr, bbox_prev, bbox_curr,seggt=None):
        I_prev_list = []
        imgs_curr = []
        bboxes_gt = []
        seggt_list = []
        for i in range(num_generated_examples):
            I_prev = self.getprevimgCrop(image_prev.copy(), bbox_prev)
            I_prev_list.append(I_prev)
        covea = 0.9
        I_currs, box_gts,segs = self.MakeTrainingExamples(4*num_generated_examples, image_curr, bbox_curr,covea, seggt)
        imgs_curr.extend(I_currs)
        bboxes_gt.extend(box_gts)
        seggt_list.extend(segs)
        return I_prev_list, imgs_curr, bboxes_gt, seggt_list
    def getprevimg(self, image_prev, bbox_prev):
        ...
        return imgcrop
    def getprevimgCrop(self, image_prev, bbox_prev, scalemin=0.3, scalemax=1.0):
        x1, y1, x2, y2 = bbox_prev
        x_center = (x1 + x2) / 2.0
        y_center = (y1 + y2) / 2.0
        w = x2 - x1
        ...
        scale = random.random()
        scalew = scalemin + scale * (scalemax - scalemin)
        scale = random.random()
        scaleh = scalemin + scale * (scalemax - scalemin)
        neww = w * scalew
        newh = h * scaleh
        xmin = random.uniform(0, w - neww) + x1
        ymin = random.uniform(0, h - newh) + y1
        xmax = xmin + neww
        ymax = ymin + newh
        if cfg.DATASET.CATDOGHORSETK.TYPE_CONTEXTBOX == 0:
            w = (xmax - xmin) * cfg.DATASET.CATDOGHORSETK.KCONTEXTFACTOR
            h = (ymax - ymin) * cfg.DATASET.CATDOGHORSETK.KCONTEXTFACTOR
        elif cfg.DATASET.CATDOGHORSETK.TYPE_CONTEXTBOX == 1:
            box_w = xmax - xmin
            box_h = ymax - ymin
            w = box_w + cfg.DATASET.CATDOGHORSETK.KCONTEXTFACTOR * (box_w + box_h)
        ...
        return imgcrop
    def MakeTrainingExamples(self, num_generated_examples, image_curr, bbox_curr,covea,seggt=None):
        imgs_curr = []
        bboxes_gt = []
        seg_crop = []
        ...
        for i in range(num_generated_examples):
            if cfg.DATASET.CATDOGHORSETK.SHIFT_MODE == 0:
                ...
                crop_box = self.shift_bbox_remo7(bbox_curr,covea)
                xmin, ymin, xmax, ymax = crop_box
                x_center = (xmin + xmax) / 2
                y_center = (ymin + ymax) / 2
                w = xmax - xmin
    ...
    def shift_bbox_remo7(self, box,covea):  # box = [xmin,ymin,xmax,ymax]
        """
        ...
        for i in range(50):
            ...
            x2 = x1 + cp_w
            y2 = y1 + cp_h
            if self.coveA(box, [x1, y1, x2, y2]) > covea:
                flagok = True
                break
        if not flagok:
            x1 = cx_box - cp_w / 2
...
if __name__ == '__main__':
    ...
    #         t = template[i].transpose((1, 2, 0))
    #         cv2.namedWindow("imgt", cv2.NORM_MINMAX)
    #         cv2.imshow("imgt", t)
    #
    #
    #
    #         imgs = search[i].transpose((1, 2, 0))
    #         loc = boxes_gt_all[i]
    #         x1, y1, x2, y2 = loc
    #         x1, y1, x2, y2 = map(int, [x1, y1, x2, y2])
    #         if label_posnegpair[i] == 1:
    ...
    # get qt data
    # """
    # cfgfile = "/home/inspur/work/siamban_GOTREMO/experiments/siamban_r50_l234/20230301DogCatHorseAsPersonV022PerTKMobileOneWeightAdd_DV05VIRandPNMod3SFT7Smin0.75Cov0.25In160NoRk_2GPU127.yaml"
    # cfg.merge_from_file(cfgfile)
    # data = BANDataset()
    ...
    #     # cnt += 1
    #     cv2.imwrite("images/%06d.z.jpg"%cnt,template)
    #     for j in range(search.shape[0]):
    #         spos = search[j].transpose((1, 2, 0))
    #         loc = boxes_gt_all[j]
    #         x1, y1, x2, y2 = loc
    #         x1, y1, x2, y2 = map(int, [x1, y1, x2, y2])
    #         if label_posnegpair[j] == 1:
    #             cv2.rectangle(spos, (x1, y1), (x2, y2), (0, 0, 255))
    #         else:
    #             cv2.rectangle(spos, (x1, y1), (x2, y2), (0, 255, 0))
    #         cv2.imwrite("images/%06d_%02d.x.jpg" %(cnt,j), spos)
    #     cnt += 1
    # exit()
    #
    # # s = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
