...
class BANDataset(Dataset):
    ...
    def MakeExamples(self, num_generated_examples, image_prev, image_curr, bbox_prev, bbox_curr,seggt=None,boxesother=[]):
        imgs_curr = []
        bboxes_gt = []
        seggt_list = []
        ...
        I_currs, box_gts,segs = self.MakeTrainingExamples(num_generated_examples - 1, image_curr, bbox_curr,seggt,boxesother)
        imgs_curr.extend(I_currs)
        bboxes_gt.extend(box_gts)
        seggt_list.extend(segs)
        return I_prev, imgs_curr, bboxes_gt, seggt_list
...
if __name__ == '__main__':
    ...
    cnt = 0
    for i in idlist:
        I_prev,search,searchboxes = data.__getitem__(i)
        for j in range(len(searchboxes)):
            if len(searchboxes[j])>0:
                imgs = search[j].transpose((1, 2, 0))
                nbox = searchboxes[j].shape[0]
                for k in range(nbox):
                    xmin,ymin,xmax,ymax = map(int,searchboxes[j][k][:4])
                    cv2.rectangle(imgs, (xmin, ymin), (xmax, ymax), (0, 255, 0))
                cv2.imwrite('images/%06d.jpg'%cnt,imgs)
                cnt+=1