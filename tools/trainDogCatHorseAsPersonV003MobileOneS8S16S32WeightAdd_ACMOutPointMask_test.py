...
from siamban.models.model_DogCatHorseAsPersonV003MobileOneS8S16S32WeightAdd_ACMOutPointMask_FrV002 import ModelBuilder
...
python ../../tools/train_PT10.py --cfg 20220920BANResNet50_uniform_2GPU126_pt1.10_syncBN.yaml --distributed-port 12355
"""
...
class BatchCollator(object):
    ...
    def __call__(self, batch):
        ...
        for sampler in batch:
            template.append(torch.from_numpy(sampler['template']))
            search.append(torch.from_numpy(sampler['search']))
            label_loc.append(torch.from_numpy(sampler['label_loc']))
            label_cls.append(torch.from_numpy(sampler['label_cls']))
            searchboxes.append(torch.from_numpy(sampler['searchboxes']))
            label_posnegpair.append(torch.from_numpy(sampler['label_posnegpair']))
            detection.append(torch.from_numpy(sampler['detection']))
            detection_bboxes.append(torch.FloatTensor(sampler['detection_bboxes']))
        template_img = torch.stack(template, dim=0)
        search_img = torch.stack(search, dim=0)
        lloc = torch.stack(label_loc, dim=0)
        lcls = torch.stack(label_cls, dim=0)
        seab = torch.stack(searchboxes,dim=0)
        lpng = torch.stack(label_posnegpair,dim=0)
        d_img = torch.stack(detection, dim=0)
        new_dict['template'] = template_img
        new_dict['search'] = search_img
        new_dict['label_loc'] = lloc
        new_dict['label_cls'] = lcls
        new_dict['searchboxes'] = seab
        new_dict['label_posnegpair']: lpng
        new_dict['detection'] = d_img
        new_dict['detection_bboxes'] = detection_bboxes
        return new_dict
