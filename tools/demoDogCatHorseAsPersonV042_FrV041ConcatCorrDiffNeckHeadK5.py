...
from siamban.models.model_DogCatHorseAsPersonV042_FrV041ConcatCorrDiffNeckHeadK5 import ModelBuilder
...
args.config = "/home/lbycdy/work/siamban/experiments/siamban_r50_l234/20230710DogCatHorseAsPersonV042_FrV041ConcatCorrDiffNeckHeadK5LocReluDV21PutPerNegMultGTAsDetCropTCat0.5Other0.1S0.65MaxS0.75_4GPU128.yaml"

args.snapshot = '/home/lbycdy/checkpoint/20230710DogCatHorseAsPersonV042_FrV041ConcatCorrDiffNeckHeadK5LocReluDV21PutPerNegMultGTAsDetCropTCat0.5Other0.1S0.65MaxS0.75_4GPU128/checkpoint_e18_param.pth'
...
# video_name = "/home/lbycdy/datasets/DogCatHorseVideo/Video1103/Dog/Dog09.mp4"
# video_name = 0
videogap = 3
fourcc = cv2.VideoWriter_fourcc('m','p','4','v')
# videofile_save = video_name.replace(".mov","_VideoGap%d.mp4"%videogap)
# out = cv2.VideoWriter(videofile_save, fourcc, 10, (1920, 1080))
...
def get_frames(video_name,videogap):
    if not video_name:
        ...
        video_name.endswith('mp4') or video_name.endswith('MP4') or video_name.endswith('LRV') or video_name.endswith('mov'):
...
def getnetinput(image,center_pos,size,flagsearch=False):
    ...
    if cfg.DATASET.CATDOGHORSETK.TYPE_CONTEXTBOX==0:
        ...
        cropw = bw + 0.5 * (bw + bh)
        croph = bh + 0.5 * (bw + bh)
        print("cfg.DATASET.CATDOGHORSETK.TYPE_CONTEXTBOX=%d"%cfg.DATASET.CATDOGHORSETK.TYPE_CONTEXTBOX)
...
def main():
    ...
    root_path = '/home/lbycdy/1'
    test_video_name = os.path.basename(args.video_name)
    save_model_path = os.path.join(root_path, test_video_name)
    cv2.namedWindow(video_name, cv2.WND_PROP_FULLSCREEN)
    fourcc = cv2.VideoWriter_fourcc(*'XVID')
    cap = cv2.VideoCapture(args.video_name)
    width = cap.get(cv2.CAP_PROP_FRAME_WIDTH)
    height = cap.get(cv2.CAP_PROP_FRAME_HEIGHT)
    xx = cap.get(cv2.CAP_PROP_FPS)
    out = cv2.VideoWriter(save_model_path, fourcc, xx, (int(width), int(height)))
    ...
    for frame in get_frames(args.video_name,videogap):
        ...
        if first_frame:
            ...
            # cap = cv2.VideoCapture('/home/lbycdy/datasets/DogCatHorseVideo/Video1103/Cat/Cat10.mp4')
            # fourcc = cv2.VideoWriter_fourcc(*'mp4v')
            # cap = cv2.cvtColor(cap,cv2.COLOR_BGR2RGBA)
            # size = (int(cap.get(cv2.CAP_PROP_FRAME_WIDTH)), int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT)))
            # fps = cap.get(cv2.CAP_PROP_FPS)
            # out = cv2.VideoWriter_fourcc('/home/lbycdy/1.mp4',fourcc ,fps, size)
            # out.release()
