...
path='/home/lbycdy/success plot/9.txt'
data = json.load(open(anno))
bbox = [0,0,0,0]
file = open(path, 'w+')
lambda_min_ratio_ = -0.7
lambda_max_ratio_ = 0.7
lambda_min_scale_ = -0.0
lambda_max_scale_ = 0.0
lambda_scale_ = 10.0
lambda_min_ratio1_ = 1.07
lambda_max_ratio1_ = 1.09
...
for imgpath in data:
    ...
    for track in data[imgpath]:
        ...
        for box in data[imgpath][track]:
            ...
            r_rand1 = lambda_min_ratio1_ + random.uniform(0, 1.0) * (lambda_max_ratio1_ - lambda_min_ratio1_)
