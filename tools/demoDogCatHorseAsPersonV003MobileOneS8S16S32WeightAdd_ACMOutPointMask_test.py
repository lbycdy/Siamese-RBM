...
args.config = "/home/lbycdy/work/siamban/experiments/siamban_r50_l234/20230103DogCatHorseAsPersonV003MobileOneS8S16S32Add_ACMOutPointMaskDV3VIRandPNMod0SFTMod1SIn160ModScale_4GPU127 .yaml"
args.snapshot = '/home/lbycdy/checkpoint/lb20230213DogCatHorseAsPersonV003MobileOneS8S16S32Add_ACMOutPointMaskDV3VIRandPNMod1TYPECONTEXT4SFTMod7SIn160ModScalenoranklossdatasetid2_4GPU120/checkpoint_e8_param.pth'
...
def main():
    ...
    for frame in get_frames(args.video_name):
        if first_frame:
            ...
            clsnew = np.zeros((160, 160))
            p = points + 80
            #  p = p.reshape((2, -1)).astype(np.int)
            p = p.T.astype(np.int)
            xs, ys = p[0], p[1]
            clsnew[ys, xs] = cls_showorg.reshape(-1)
            clsnew = clsnew.astype(np.uint8)
            cv2.namedWindow("clsneworg", cv2.NORM_MINMAX)
            cv2.imshow("clsneworg", clsnew)
            heat_img = cv2.applyColorMap(clsnew, cv2.COLORMAP_JET)
            add_img = cv2.addWeighted(imgshow, 0.3, heat_img, 0.7, 0)
            cv2.namedWindow("clsnew", cv2.NORM_MINMAX)
            cv2.imshow("clsnew", add_img)
