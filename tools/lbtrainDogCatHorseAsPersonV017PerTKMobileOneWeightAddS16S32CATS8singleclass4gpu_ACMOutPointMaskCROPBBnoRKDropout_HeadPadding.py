...
from siamban.models.lbmodel_DogCatHorseAsPersonV017PerTKMobileOneWeightAddS16S32CATS8singleclass4gpu_ACMOutPointMaskDropHWCenterRK_HeadPadding import ModelBuilder,AdjustAllLayerFPNAddFlagCenterCrop,RPNHead
...
from siamban.models.backbone.mobileone_strideS16S32OutTwo import reparameterize_model_train,reparameterize_model_allskipscale
...
def build_opt_lr(model, current_epoch=0):
    ...
    # trainable_params += [{'params': filter(lambda x: 'depthcorr' not in x.named_parameters(),model.head.parameters()),'lr': cfg.TRAIN.BASE_LR, 'weight_decay': cfg.TRAIN.WEIGHT_DECAY_BB}]
    # trainable_params += [{'params': model.head.depthcorr.parameters(),
    #                       'lr': cfg.BACKBONE.LAYERS_LR * cfg.TRAIN.BASE_LR}]
    param_head = []
    param_head_new = []
    for name, p in model.head.named_parameters():
        if 'depthcorr' in name:
            param_head_new.append(p)
        else:
            param_head.append(p)

    trainable_params += [{'params': param_head_new,
                          'lr': cfg.BACKBONE.LAYERS_LR * cfg.TRAIN.BASE_LR}]

    trainable_params += [{'params': param_head,
...
class Trainer:
    ...
    def init_model(self):
        ...
        if cfg.BACKBONE.PRETRAINED:
            ...
            self.model.neck = AdjustAllLayerFPNAddFlagCenterCrop(in_channels=[256,1024],out_channels=[192,192],center_size=9).to(self.rank)
            self.model.head = RPNHead(cin=192).to(self.rank)
            ...
            #     print('222222222222221111',key1)
            # for key2 in pretrained['state_dict']:
            #     print('22222222222222',key2)
            # exit()
            state_dict1 = self.model.neck.state_dict()
            # for key2 in state_dict1:
            #     print('3333333333333',key2)
            state_dict2 = self.model.head.state_dict()
            # model.head.parameters()
            for key_backbone in state_dict:
                keybackbone = "backbone." + key_backbone
                state_dict[key_backbone] = pretrained[keybackbone]
            logger.info(self.model.backbone.load_state_dict(state_dict, strict=True))
            for key_neck in state_dict1:
                keyneck = "neck." + key_neck
                state_dict[key_neck] = pretrained[keyneck]
            logger.info(self.model.neck.load_state_dict(state_dict1, strict=True))
            # exit()
            for key_head in state_dict2:
                if key_head[:9] == 'depthcorr':
                    keyhead = "head." + key_head
                    state_dict[key_head] = pretrained[keyhead]

            logger.info(self.model.head.load_state_dict(state_dict2, strict=True))
        ...
        # load pretrain
            ...
            load_pretrain(self.model.backbone, cfg.TRAIN.PRETRAINED)
