...
from siamban.models.lbmodel_DogCatHorseAsPersonV008PerTKMobileOneWeightAdd_ACMOutPointMask_AnchorTarget_HeadPadding import ModelBuilder
...
import numpy as np
import math
from siamban.models.lbmodel_DogCatHorseAsPersonV008PerTKMobileOneWeightAdd_ACMOutPointMask_AnchorTarget_HeadPadding import Anchors
...
args.config = "/home/lbycdy/work/siamban/experiments/siamban_r50_l234/lb20230306DogCatHorseAsPersonV008PerTKMobileOneWeightAdd_DV05VIRandPNMod3SFT7Smin0.75Cov0.25In160NoRkCropbb_4GPU127.yaml"
# args.config = "/home/lbycdy/work/siamban/experiments/siamban_r50_l234/lb20230313DogCatHorseAsPersonV008PerTKMobileOneWeightAdd_DV08VIRandAT1SFT7Smin0.75Cov0.35In160NoRkCropbb_4GPU122.yaml"

args.snapshot1 = '/home/lbycdy/checkpoint/lb20230306DogCatHorseAsPersonV008PerTKMobileOneWeightAdd_DV05VIRandPNMod3SFT7Smin0.75Cov0.25In160NoRkCropbb_4GPU127/checkpoint_e18_param.pth'
args.snapshot2 = '/home/lbycdy/checkpoint/lb20230306DogCatHorseAsPersonV008PerTKMobileOneWeightAdd_DV05VIRandPNMod3SFT7Smin0.75Cov0.25In160NoRkCropbb_4GPU127/checkpoint_e13_param.pth'
...
# video_name = "/home/lbycdy/datasets/DogCatHorseVideo/Video1103/Dog/Dog04.mp4"
...
def _convert_bbox1(delta, anchor):
    delta = delta.reshape(1, 4, -1, 19, 19).permute(1, 2, 3, 4, 0).contiguous().view(4, -1)
    delta = delta.detach().cpu().numpy()
...
def _convert_score1(score):
    ...
    #     score = score.sigmoid().detach().cpu().numpy()
    # else:
    score = score.reshape(1, 2, -1, 19, 19).permute(1, 2, 3, 4, 0).contiguous().view(2, -1).permute(1, 0)
    if cfg.TRAIN.FLAG_SIGMOID_LOSS:
        # score = score.sigmoid()
        score = score.detach()[:, 1].cpu().numpy()
    else:
...
def main():
    ...
    for frame in get_frames(args.video_name,videogap):
        if first_frame:
            ...
            # add_img1 = cv2.addWeighted(imgshow2, 0.5, heat_img1, 0.5, 0)
            ...
            for i in range(cfg.ANCHOR.ANCHOR_NUM, cfg.ANCHOR.ANCHOR_NUM * 2):
                ...
                add_img = cv2.addWeighted(imgshow1, 0.5, heat_img, 0.5, 0)
            ...
            score1 = _convert_score1(cls1)

            pred_bbox1 = _convert_bbox1(loc1, anchors)
            scoretmp1 = score1.copy()
            idx1 = scoretmp1.argsort()[::-1]


            score2 = _convert_score1(cls2)
            pred_bbox2 = _convert_bbox1(loc2, anchors)
            ...
            sort_pscore1 = pscore1.argsort()[::-1]
            best_idx1 = sort_pscore1[0]
            ...
            sort_pscore2 = pscore2.argsort()[::-1]
            best_idx2 = sort_pscore2[0]
