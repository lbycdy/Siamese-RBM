...
class PointTarget2_Rect:
    def __init__(self, stride,outputsize,scalepos):
        self.points = Point(stride,outputsize, 80)#160/11=14.5
...
def coveA(box1, box2):
    ...
    area1 = (x2_1 - x1_1) * (y2_1 - y1_1)
    return float(area_inter) / float(area1 + 1e-8)
# file = '/home/lbycdy/datasets/OCean/GOT10K/GOT10KDogCatHorse_CropnotailWithNeedTailAnno.json'
# imgroot = '/home/lbycdy/datasets/OCean/GOT10K/crop511'
file = '/home/lbycdy/datasets/COW/make_datasets.json'
imgroot = '/home/lbycdy/datasets/COW/crop511'
# fhlines = open(file).readlines()
data = json.load(open(file))
netimgsize = 160
bbox = [0,0,0,0]
# point_target1_s16out13 = PointTarget1(8,13,netimgsize/2)
# point_target1_s8out19 = PointTarget1(8,19,80)
point_target1_s16out13_1 = PointTarget2_Rect(12,13,3)
point_target1_s16out13 = PointTarget2_Rect(8,13,3)
point_target1_s8out19 = PointTarget2_Rect(8,19,3)
...
for video in data:
    for track in data[video]:
        ...
        for frame in data[video][track]:
            ...
            box[0] = 511 * box[0]
            box[1] = 511 * box[1]
            box[2] =511 * box[2]+box[0]
            box[3] = 511 * box[3]+box[1]
            ...
            imgpath = os.path.join(video, path_format.format(frame, track, 'x'))
            img = cv2.imread(os.path.join(imgroot,imgpath))
            box = get_bbox(img, box)
            bbox[0] = int(box.x1)
            bbox[1] = int(box.y1)
            bbox[2] = int(box.x2)
            bbox[3] = int(box.y2)
            x1,y1,x2,y2,flagok = shift_bbox_remo(bbox)
            ...
            cv2.imwrite('/home/lbycdy/1.jpg',imgcrop160show)
            cv2.imshow("clsorgs8out11img", imgcrop160show)

            cls, delta, delta_weight = point_target1_s16out13_1(bbox, outsize, False)
            clshow = cls * 255
            clshow = clshow.astype(np.uint8)
            cv2.namedWindow("clsorgs8out11", cv2.NORM_HAMMING)
            cv2.imshow("clsorgs8out11", clshow)
            imgcrop160show = imgcrop160.copy()
            points = point_target1_s16out13_1.points.points
            for i in range(outsize):
                for j in range(outsize):
                    cid = cls[i, j]
                    x, y = points[:, i, j]
                    ...
                    if cid == 1:
                        ...
                        cv2.circle(imgcrop160show, (x, y), 3, (0, 255, 0), -1)
            cv2.namedWindow("clsorgs8out11_1img", cv2.NORM_HAMMING)
            cv2.imwrite('/home/lbycdy/3.jpg', imgcrop160show)
            cv2.imshow("clsorgs8out11img_1", imgcrop160show)
            ...
            for i in range(19):
                for j in range(19):
                    ...
                    x, y = int(x), int(y)
                    if cid == 1:
                        cv2.circle(imgcrop160show, (x, y), 2, (0, 0, 255), -1)
                    else:
                        cv2.circle(imgcrop160show, (x, y), 2, (0, 255, 0), -1)
            cv2.namedWindow("clsorgs8out19img", cv2.NORM_HAMMING)
            cv2.imwrite('/home/lbycdy/2.jpg', imgcrop160show)
            cv2.imshow("clsorgs8out19img", imgcrop160show)

            # cls, delta, delta_weight = point_target3_squaremins8_S4(bbox, 11, False)
            # clshow = cls * 255
            # clshow = clshow.astype(np.uint8)
            # cv2.namedWindow("clss8_s4", cv2.NORM_HAMMING)
            # cv2.imshow("clss8_s4", clshow)
            #
            # imgcrop160show = imgcrop160.copy()
            # points = point_target3_squaremins8_S4.points.points
            # print(points.min(),points.max())
            # for i in range(11):
            #     for j in range(11):
            #         cid = cls[i, j]
            ...
            #         else:
            #             cv2.circle(imgcrop160show, (x, y), 3, (0, 255, 0), -1)
            # cv2.namedWindow("clss8_s4img", cv2.NORM_HAMMING)
            # cv2.imshow("clss8_s4img", imgcrop160show)


            # cls, delta,delta_weight = point_target3_squaremins10_S4(bbox, 11, False)
            # clshow = cls * 255
            # clshow = clshow.astype(np.uint8)
            # cv2.namedWindow("clss10_s4", cv2.NORM_HAMMING)
            # cv2.imshow("clss10_s4", clshow)
            #
            # imgcrop160show = imgcrop160.copy()
            # points = point_target3_squaremins10_S4.points.points
            # for i in range(11):
            #     for j in range(11):
            #         cid = cls[i, j]
            #         x, y = points[:, i, j]
            ...
            #         else:
            #             cv2.circle(imgcrop160show, (x, y), 3, (0, 255, 0), -1)
            # cv2.namedWindow("clss10_s4img", cv2.NORM_HAMMING)
            # cv2.imshow("clss10_s4img", imgcrop160show)
            #
            #
            #
            # cls, delta, delta_weight = point_target3_squaremins10_S3_5(bbox, 11, False)
            # clshow = cls * 255
            # clshow = clshow.astype(np.uint8)
            # cv2.namedWindow("clss10_s3.5", cv2.NORM_HAMMING)
            # cv2.imshow("clss10_s3.5", clshow)
            #
            # imgcrop160show = imgcrop160.copy()
            # points = point_target3_squaremins10_S3_5.points.points
            # for i in range(11):
            #     for j in range(11):
            #         cid = cls[i, j]
            #         x, y = points[:, i, j]
            ...
            #         else:
            #             cv2.circle(imgcrop160show, (x, y), 3, (0, 255, 0), -1)
            # cv2.namedWindow("clss10_s3.5img", cv2.NORM_HAMMING)
            # cv2.imshow("clss10_s3.5img", imgcrop160show)

            # cls, delta, delta_weight = point_target3_squaremins14_S4(bbox, 11, False)
            # clshow = cls * 255
            # clshow = clshow.astype(np.uint8)
            # cv2.namedWindow("clss14_s4", cv2.NORM_HAMMING)
            # cv2.imshow("clss14_s4", clshow)
            #
            # imgcrop160show = imgcrop160.copy()
            # points = point_target3_squaremins14_S4.points.points
            # for i in range(11):
            #     for j in range(11):
            #         cid = cls[i, j]
            #         x, y = points[:, i, j]
            ...
            #         else:
            #             cv2.circle(imgcrop160show, (x, y), 3, (0, 255, 0), -1)
            # cv2.namedWindow("clss14_s4img", cv2.NORM_HAMMING)
            # cv2.imshow("clss14_s4img", imgcrop160show)
            #
            # cls, delta, delta_weight = point_target2_rect10_S4(bbox, 11, False)
            # clshow = cls * 255
            # clshow = clshow.astype(np.uint8)
            # cv2.namedWindow("clsrect10_S4", cv2.NORM_HAMMING)
            # cv2.imshow("clsrect10_S4", clshow)
            #
            # imgcrop160show = imgcrop160.copy()
            # points = point_target2_rect10_S4.points.points
            # for i in range(11):
            #     for j in range(11):
            #         cid = cls[i, j]
            #         x, y = points[:, i, j]
            ...
            #         else:
            #             cv2.circle(imgcrop160show, (x, y), 3, (0, 255, 0), -1)
            # cv2.namedWindow("clsrect10_S4img", cv2.NORM_HAMMING)
            # cv2.imshow("clsrect10_S4img", imgcrop160show)
