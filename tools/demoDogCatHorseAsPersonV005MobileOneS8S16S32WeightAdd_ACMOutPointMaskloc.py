...
from siamban.models.model_DogCatHorseAsPersonV005MobileOneS8S16S32WeightAdd_ACMOutPointMaskloc_FrV002 import ModelBuilder
...
args.config = "/home/lbycdy/work/siamban/experiments/siamban_r50_l234/lb20230226DogCatHorseAsPersonV005MobileOneS8S16S32Add_ACMOutPointMasklocDV3VIRandPNMod1TYPECONTEXT0SFTMod2SIn160datasetid2_4GPU122.yaml"
args.snapshot = '/home/lbycdy/checkpoint/lb20230226DogCatHorseAsPersonV005MobileOneS8S16S32Add_ACMOutPointMasklocDV3VIRandPNMod1TYPECONTEXT0SFTMod2SIn160datasetid2_4GPU122/checkpoint_e19_param.pth'
...
def main():
    ...
    for frame in get_frames(args.video_name,videogap):
        if first_frame:
            ...
            print(x_crop.size(), ":x_crop1")




            cls,loc,loc2 = model.track(x_crop)
            cls_show = cls.clone()
            ...
            head = cv2.resize(cls_show, (cfg.TRAIN.SEARCH_SIZE, cfg.TRAIN.SEARCH_SIZE))
            head *= 255
            head = head.astype(np.uint8)
            heat_img = cv2.applyColorMap(head, cv2.COLORMAP_JET)
            ...
            prior_center_x = 0.5
            prior_center_y = 0.5
            bbox_center_x = loc2[0] * prior_variance[0] * prior_width + prior_center_x
            bbox_center_y = loc2[1] * prior_variance[1] * prior_height + prior_center_y
            bbox_width = math.exp(loc2[2] * prior_variance[2]) * prior_width
            bbox_height = math.exp(loc2[3] * prior_variance[3]) * prior_height
            ...
            cx1 = bbox[0] + center_pos[0] - size[0] * cfg.DATASET.CATDOGHORSETK.KCONTEXTFACTOR // 2
            cy1 = bbox[1] + center_pos[1] - size[1] * cfg.DATASET.CATDOGHORSETK.KCONTEXTFACTOR // 2
            ...
            cv2.putText(frame, "%f" % 1, (int(cx), int(cy)), cv2.FONT_HERSHEY_COMPLEX, fontScale=1,
                        color=(0, 0, 255), thickness=1)
            cv2.putText(frame, "pre+loc", (int(bbox[0]), int(bbox[1])), cv2.FONT_HERSHEY_COMPLEX, fontScale=3,
                        color=(0, 124, 234), thickness=1)
